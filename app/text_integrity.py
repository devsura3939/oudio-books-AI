...
def translation_is_valid(source, candidate, target):
    ...
    if target == "ka":
        ...
        if re.search(r'(?<![\u10A0-\u10FF])[ა-ჰ]+ები\s+(?:[ა-ჰ]+\s+)?(?:იპოვეს|თქვეს|გააკეთეს|დაწერეს|წაიკითხეს|ნახეს|გახსნეს|დახურეს)(?![\u10A0-\u10FF])', candidate):
            return False
        if re.search(r'(?<![\u10A0-\u10FF])(?:გამოიყურებოდა\s+გარეგნულად|თავის\s+გარეგნულ\s+ბიძგში|ქვებივით\s+აფრინდა\s+მათ|გიმ(?:რ|კრ)ეკის|საკმარისად\s+იყო\s+ნაპოვნი|თავსატეხების\s+ყუთებ|უაზრობის\s+კოშმარი\s+უსასრულოდ|უგემოვნო\s+ზღვაში|უცოდინარი\s+ჭეშმარიტებების|უბიძგებდა\s+მუდამ\s+გარეგნულად|თავსატეხების\s+ყუთები\s+მათში|ადამიანებს\s+არ\s+ჰქონდათ\s+მარტივი\s+წვდომა)(?![\u10A0-\u10FF])', candidate):
            return False
...
def polish_georgian_literary_syntax(text: str) -> str:
    ...
    literary_calque_repairs = [
        ...
        (r'(?<![\u10A0-\u10FF])(?:კაცებსა\s+და\s+ქალებს\s+)?(?:არ\s+ჰქონდათ|არ\s+ჰქონდა)\s+(?:მარტივი|ადვილი)\s+წვდომა(?![ა-ჰ])', 'ადამიანებს ხელი არ მიუწვდებოდათ'),
