...
def translation_is_valid(source, candidate, target):
    ...
    if target == "ka":
        ...
        if re.search(r'(?<![\u10A0-\u10FF])[ა-ჰ]+ები\s+(?:[ა-ჰ]+\s+)?(?:იპოვეს|თქვეს|გააკეთეს|დაწერეს|წაიკითხეს|ნახეს|გახსნეს|დახურეს)(?![\u10A0-\u10FF])', candidate):
            return False
        if re.search(r'(?<![\u10A0-\u10FF])(?:გამოიყურებოდა\s+გარეგნულად|თავის\s+გარეგნულ\s+ბიძგში|ქვებივით\s+აფრინდა\s+მათ|გიმ(?:რ|კრ)ეკის|საკმარისად\s+იყო\s+ნაპოვნი|თავსატეხების\s+ყუთებ|უაზრობის\s+კოშმარი\s+უსასრულოდ)(?![\u10A0-\u10FF])', candidate):
            return False
