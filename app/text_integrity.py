...
def translation_is_valid(source, candidate, target):
    ...
    if target == "ka":
        ...
        if re.search(r'(?<![\u10A0-\u10FF])(?:კარი\s+დაეჯახა|იატაკი\s+ყვიროდა|კბილები\s+ლაპარაკობდნენ|ნაბიჯები\s+ექოდ\s+ისმოდა|დააკაკუნა\s+კარებზე|დაკარგა\s+ტემპერამენტი|შეინარჩუნა\s+სიგრილე|დაჯდა\s+მის\s+ნერვებზე|გული\s+დადო\s+ამაზე|გული\s+გატეხა|იჯდა\s+გადაჯვარედინებული\s+ფეხებით|იდგა\s+ფეხის\s+თითებზე|დაეცა\s+მის\s+მუხლებზე|გადააჯვარედინა\s+თავისი\s+ხელები|დახარა\s+თავისი\s+თავი|დიდით\s+და\s+ვრცელით|ერთხელ\s+ლურჯ\s+მთვარეზე|გასწვრივ\s+და\s+გასწვრივ|უსაფრთხოდ\s+და\s+ხმით|ნელა,\s*მაგრამ\s+დარწმუნებით|მობრუნდა\s+უკან|მოშორებით\s+შებრუნდა|მოშორებით\s+შეხედა|მხედველობა\s+დაიჭირა)(?![\u10A0-\u10FF])', candidate):
            return False
        if re.search(r'(?<![\u10A0-\u10FF])(?:წვიმამ\s+დაიწყო\s+მძიმედ|წვიმა\s+წამოვიდა\s+მძიმედ|ჭექა-ქუხილმა\s+გააგორა|ნისლი\s+დაჯდა\s+ხეობაზე|ქარი\s+ყვიროდა\s+ხეებში|მზე\s+სცემდა\s+დაუნდობლად|ცეცხლმა\s+თავი\s+დაწვა|მდინარე\s+გაიქცა\s+მშრალი|ტიკტიკით\s+გაუშვა\s+წუთები|შუშა\s+დაიმსხვრა\s+ნაჭრებში|გაფერმკრთალდა\s+სიჩუმეში|იყო\s+სიტყვების\s+დანაკარგში|გონება\s+ერთხელ\s+და\s+ყველასთვის|ჰქონდა\s+ცუდი\s+გრძნობა\s+ამაზე|ეს\s+გულთან\s+აიღო|თავის\s+გვერდით\s+იყო\s+სიბრაზით|ეს\s+გათენდა\s+მასზე|მოვიდა\s+თავის\s+გრძნობებში|დაინახა\s+მოტყუების\s+გავლით|მიიღო\s+გარანტირებულად|დაკარგა\s+დროის\s+კვალი|ღრმად\s+ფიქრში|ლურჯიდან\s+გარეთ|ყველა\s+შანსების\s+წინააღმდეგ|ეჭვის\s+ჩრდილის\s+მიღმა|ზემოდან\s+ქვემომდე)(?![\u10A0-\u10FF])', candidate):
            return False
        if re.search(r'(?<![\u10A0-\u10FF])(?:მძაფრი\s+სუნი\s+ეკიდა|სუნი\s+ეკიდა\s+ჰაერში|სიცივემ\s+გაიარა\s+მის\s+ხერხემალში|ზარი\s+მის\s+ყურებში|ყურებში\s+რეკვა\s+დაიწყო|დატოვა\s+მწარე\s+გემო|სიცივემ\s+გაჭრა\s+ძვალამდე|ყველა\s+ანგარიშით|არ\s+აქვს\s+მნიშვნელობა\s+რა\s+მოხდება|სხვა\s+სიტყვებში|სიმართლე\s+სათქმელად|უკან\s+ყურების\s+გარეშე\s+გაიქცა|გაიქცა\s+უკან\s+ყურების\s+გარეშე|გააკეთა\s+თავისი\s+გზა|თავისი\s+გზა\s+გააკეთა|მიჰყვა\s+ცხელ\s+ქუსლებზე|გაიარა\s+გვერდით\s+გარშემო|მიჰყვა\s+თავისი\s+თვალებით|გაჭრა\s+მისი\s+სიტყვა|შეაწყვეტინა\s+იგი|დაარტყა\s+საუბარი|ილაპარაკა\s+უკან|გამოხატა\s+აზრი|ილაპარაკა\s+თავისი\s+გონება|დრო\s+გადიოდა\s+გარეთ|დღის\s+გავლით|გაიყინა\s+ადგილზე\s+ერთ\s+წამში|დაიჭირა\s+მომენტი|ძლივს\s+დაიჭირა\s+თავისი\s+სუნთქვა)(?![\u10A0-\u10FF])', candidate):
            return False
        if re.search(r'(?<![\u10A0-\u10FF])(?:მზე\s+ჩადიოდა\s+ჰორიზონტზე|ჩრდილი\s+დაეცა\s+მიწაზე|მთვარე\s+ანათებდა\s+კაშკაშად|ბინდი\s+დაეცა\s+ქალაქს|სხივებმა\s+გაჭრა\s+ღრუბლები|დილის\s+სინათლე\s+გატყდა|კარი\s+თავისით\s+გაიღო|ფანჯარა\s+გატყდა\s+თვითონ|ყინული\s+გადნა\s+თავისით|სახლი\s+დაიწვა\s+თვითონ|წყალმა\s+დაიხრჩო\s+იგი|ტოტი\s+გატყდა\s+თავისით|მოვიდა\s+შიგნით|წავიდა\s+გარეთ|ჩამოვიდა\s+დაბლა|ავიდა\s+ზემოთ|გადავიდა\s+მეორე\s+მხარეს|გამოვიდა\s+გარეთ|სრულიად\s+აშკარად|ძალიან\s+მარტივად|უკიდურესად\s+რთული|საერთოდ\s+არ\s+ეშინოდა|თითქმის\s+შეუძლებელი|აბსოლუტურად\s+დარწმუნებული|და\s+მერე\s+მან\s+თქვა|როდესაც\s+ყველაფერი\s+დასრულდა|არც\s+კი\s+შეუხედავს|ამასობაში\s+კი|ერთი\s+სიტყვით\s+რომ\s+ვთქვათ)(?![\u10A0-\u10FF])', candidate):
            return False
...
def polish_georgian_literary_syntax(text: str) -> str:
    ...
    postpositional_fixes = [
        ...
        (r'(?<!მოვიდა\s)(?<!შემოვიდა\s)(?<![\u10A0-\u10FF])შიგნით\s+ოთახში(?![ა-ჰ])', 'ოთახში'),
    ...
    for pat, repl in dialogue_locution_fixes:
        t = re.sub(pat, repl, t)
    ...
    for pat, repl in causative_inchoative_fixes:
        t = re.sub(pat, repl, t)

    # 78. Intransitive Directional Particle Preverbs (მიმართულებითი ზმნისწინის სიჭარბის აღმოფხვრა)
    directional_particle_fixes = [
        (r'(?<![\u10A0-\u10FF])(?:მოვიდა\s+შიგნით\s+ოთახში|მოვიდა\s+ოთახში)(?![ა-ჰ])', 'ოთახში შემოვიდა'),
    ...
    for pat, repl in narrative_connective_fixes:
        t = re.sub(pat, repl, t)
