...
const schema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    language,
    scope: z.enum(["translate", "transcribe", "both"]),
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (input.action === "overview") {
          const [pack, cases] = await Promise.all([
            loadActivePack(input.language),
            loadBenchmark(input.language),
          ]);
          const evaluated = evaluatePack(pack.items, cases);
          const [{ data: versions }, { data: keys }, { data: sessions }, { data: caseRows }] =
            await Promise.all([
              sb
                .from("engine_versions")
                .select("id, version, score, note, source, model, created_at")
              ...
              sb
                .from("training_keys")
                .select(
                  "id, key_prefix, label, language, scope, uses, last_used_at, revoked_at, created_at",
                )
              ...
              sb
                ...
                .select(
                  "id, language, driver, model, status, iterations, accepted, start_score, current_score, started_at, finished_at, summary",
                )
                .eq("language", input.language)
              ...
              sb
                ...
                .select("id, kind, source, expected, origin, note, created_at")
                .eq("language", input.language)
                .order("created_at", { ascending: false })
                .limit(200),
            ]);
        ...
        if (input.action === "rewind") {
          ...
          const { error } = await sb
            .from("engine_active")
            .upsert(
              { language: input.language, version_id: target.id, enabled: true },
              { onConflict: "language" },
            );
          if (error) return jsonResponse({ error: error.message }, 500);
        ...
        if (input.action === "session") {
          ...
          const { data: iterations } = await sb
            ...
            .select(
              "id, idx, model, accepted, reason, score_before, score_after, proposal, created_at",
            )
        ...
        if (!cases.length)
          return jsonResponse({ error: "Add benchmark cases before training" }, 400);

        const model = input.model || process.env["TRAINING_MODEL"] || MODEL;
        ...
        try {
          for (let i = 1; i <= input.iterations; i += 1) {
            ...
            const evaluated = evaluatePack(pack.items, cases);
            if (evaluated.passed === evaluated.total) break;
            ...
            if (!upstream.ok) {
              throw new Error(
                `Provider request failed (${upstream.status}). Active rules were retained.`,
              );
            }
            ...
            if (data.choices?.[0]?.finish_reason !== "stop")
              throw new Error("Incomplete model response; no truncated proposal accepted.");
            let items: unknown[] = [];
            try {
            ...
            unchanged = outcome.accepted ? 0 : unchanged + 1;
            log.push({
              iteration: i,
              accepted: outcome.accepted,
              reason: outcome.reason,
          ...
          const detail = request.signal.aborted
            ? "Cancelled; confirmed rules were retained."
          ...
          await sb
            ...
            .eq("id", session.id);
          return jsonResponse({ error: detail, session_id: session.id, log }, 502);
        }
        ...
        await sb
          ...
          .update({
            status: "finished",
            finished_at: new Date().toISOString(),
            current_score: finalScore.score,
          })
          .eq("id", session.id);
        return jsonResponse({
          session_id: session.id,
          ...
          pack_version: finalPack.version,
        });
      },
    },
  },
...
Previous proposal feedback: ${feedback.slice(0, 800)}
...
${JSON.stringify(
  cases.slice(0, 12).map((c) => ({ kind: c.kind, source: c.source, expected: c.expected })),
  null,
  1,
)}

Return JSON exactly as: { "items": [ ...at most 6 rules... ], "note": "one sentence" }
