...
• BENCHMARK LITERARY PARALLEL SENTENCES:
  ...
  - "What is man if his chief good and market of his time be but to sleep and feed? A beast, no more."
    → „რა არის კაცი, თუკი მისი უზენაესი სიკეთე და ცხოვრების აზრი მხოლოდ ჭამა და ძილია? პირუტყვია და სხვა არაფერი.“ (Ilia Chavchavadze / ილია ჭავჭავაძე, კაცია-ადამიანი?!)
  - "A hero is known on the battlefield; a sage in counsel."
    → „გმირი ბრძოლის ველზე იცნობა, ბრძენი კი — განსჯაში.“ (Akaki Tsereteli, Bashi-Achuki / აკაკი წერეთელი, ბაში-აჩუკი)
  - "The guest is from God; his shelter and honor are sacred above life itself."
    → „სტუმარი ღვთისაა, მისი პატივისცემა და შეფარება სიცოცხლეზე მეტია.“ (Vazha-Pshavela, Host and Guest / ვაჟა-ფშაველა, სტუმარ-მასპინძელი)
  - "A brave man sheds a tear for a fallen foe, for courage recognizes courage across all enmity."
...
• NARRATIVE FLOW & DISCOURSE PATTERNS:
  ...
  - Natural Rhythm: Vary sentence length; avoid stacking multiple subordinate clauses with „რომელიც“; prefer active aorists over passive participle constructions.`;
...
// KA-123 v1.49.0 — "Georgian Pro" Syntactic & Polypersonal Engine.
const KA_SYNTACTIC_POLYPERSONAL_ENGINE = `
"GEORGIAN PRO" SYNTACTIC & POLYPERSONAL ENGINE (KA-123):
• POSTPOSITION VOWEL SYNCOPATION & TRUNCATION (კუმშვა და კვეცა):
  - Consonant-final nouns: Drop nominative -ი before postpositions:
    * ქალაქი + -ში → ქალაქში (NEVER *ქალაქიში).
    * წიგნი + -ში → წიგნში (NEVER *წიგნიში).
    * კაცი + -თან → კაცთან (NEVER *კაცითან).
    * ოთახი + -დან → ოთახიდან (NEVER *ოთახიიდან).
  - Internal stem syncopation (კუმშვა) in oblique cases and postpositions:
    * წყალი → წყლის, წყლით, წყლიდან (NEVER *წყალის, *წყალიდან).
    * მგელი → მგლის, მგლით, მგლისგან (NEVER *მგელის).
    * ქვეყანა → ქვეყნის, ქვეყნით, ქვეყნიდან (NEVER *ქვეყანის).
  - Vowel-stem stability:
    * მთა → მთაში, მთაზე, მთიდან.
    * დედა → დედასთან, დედისთვის.
    * ხე → ხეზე, ხეში, ხიდან.
• SCREEVE SERIES AGREEMENT (სერიათა შეწყობა):
  - Series I (Present/Future):
    Subject takes NOMINATIVE (-ი / vowel); Direct Object takes DATIVE (-ს).
    მაგალითად: ბიჭი წიგნს კითხულობს / უფლისწული ვარდს უყურებს.
  - Series II (Aorist / Simple Past):
    Transitive Subject takes ERGATIVE (-მა / -მ); Direct Object takes NOMINATIVE (-ი / vowel).
    მაგალითად: ბიჭმა წიგნი წაიკითხა / უფლისწულმა ვარდი დაინახა.
    Intransitive Subject stays NOMINATIVE: ბიჭი მოვიდა / უფლისწული წავიდა.
  - Series III (Evidential / Perfect):
    Subject takes DATIVE (-ს); Direct Object takes NOMINATIVE (-ი / vowel).
    მაგალითად: ბიჭს წიგნი წაუკითხავს / უფლისწულს ვარდი უნახავს.
• INVIOLABLE NEGATIVE IMPERATIVE LOGIC:
  - NEVER use declarative არ with an imperative form:
    * Don't go! → ნუ წახვალ! (NEVER *არ წახვიდე!).
    * Don't be afraid! → ნუ გეშინია! (NEVER *არ შეგეშინდეს!).
    * Don't cry! → ნუ ტირი! (NEVER *არ იტირო!).
    * Don't forget! → ნუ დაივიწყებ! (NEVER *არ დაივიწყო!).
  - Negation triple distinction:
    * არ = declarative refusal or negation of fact.
    * ვერ = physical or circumstantial inability.
    * ნუ = prohibition and negative command.
• EXPERIENCER DATIVE INVERSION:
  - Emotional, physical, sensation, and cognitive states MUST use inverted dative experiencer:
    * მშია (I am hungry), მას შია (he is hungry).
    * მცივა (I am cold), მას სცივა (he is cold).
    * მტკივა (I feel pain), მას სტკივა (he feels pain).
    * მიყვარს (I love), მას უყვარს (he loves).
    * მახსოვს (I remember), მას ახსოვს (he remembers).
    * მჭირდება (I need), მას სჭირდება (he needs).
    * მინდა (I want), მას უნდა (he wants).
...
// ── 4.6. DETERMINISTIC MORPHOSYNTACTIC CASE & SYNTAX SYNTHESIZER ─────────────
// Synthesizes authentic Georgian cases (Ergative for transitive aorist subjects,
// Dative for prepositional/indirect objects and experiencers, Genitive for possessives),
// handles postposition vowel syncopation (კუმშვა/კვეცა), and fixes calqued syntax.
function synthesizeGeorgianMorphology(text) {
    ...
    const ergativeReplacements = [
        ...
        [new RegExp(`(?<![\\u10A0-\\u10FF])მეფე(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'მეფემ'],
        [new RegExp(`(?<![\\u10A0-\\u10FF])მელა(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'მელამ'],
        [new RegExp(`(?<![\\u10A0-\\u10FF])კაცი(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'კაცმა'],
        [new RegExp(`(?<![\\u10A0-\\u10FF])ბავშვი(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'ბავშვმა'],
        [new RegExp(`(?<![\\u10A0-\\u10FF])ბიჭი(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'ბიჭმა'],
        [new RegExp(`(?<![\\u10A0-\\u10FF])გოგო(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'გოგომ'],
        [new RegExp(`(?<![\\u10A0-\\u10FF])ქალი(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'ქალმა'],
        [new RegExp(`(?<![\\u10A0-\\u10FF])ავტორი(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'ავტორმა'],
        [new RegExp(`(?<![\\u10A0-\\u10FF])ვარდი(?=\\s+${aoristVerbs})(?![ა-ჰ])`, 'g'), 'ვარდმა'],
    ...
    out = out.replace(/(?<![\u10A0-\u10FF])წყალ(ის|ით|იდან|ისკენ|ისთვის)(?![ა-ჰ])/g, 'წყლ$1');
    out = out.replace(/(?<![\u10A0-\u10FF])მგელ(?:ი)?-?ის(?![ა-ჰ])/g, 'მგლის');
    out = out.replace(/(?<![\u10A0-\u10FF])მგელ(?:ი)?-?დან(?![ა-ჰ])/g, 'მგლიდან');
    out = out.replace(/(?<![\u10A0-\u10FF])მგელ(ის|ით|იდან|ისკენ|ისთვის)(?![ა-ჰ])/g, 'მგლ$1');
    out = out.replace(/(?<![\u10A0-\u10FF])ქვეყან(?:ა)?-?(ში|ზე|თან)(?![ა-ჰ])/g, 'ქვეყანაში');
    out = out.replace(/(?<![\u10A0-\u10FF])ქვეყან(?:ა)?-?(დან|ის|ით)(?![ა-ჰ])/g, 'ქვეყნიდან');

    // Consonant stems drop nominative -ი before -ში, -ზე, -თან (ქალაქში, წიგნში, კაცთან, ოთახში)
    out = out.replace(/([ა-ჰ]+[ბგდვზთკლმნპჟრსტუფქღყშჩცძწჭხჯჰ])ი-?(ში|ზე|თან)(?![ა-ჰ])/g, '$1$2');
    // Consonant stems attach -იდან (ქალაქიდან, წიგნიდან)
    out = out.replace(/([ა-ჰ]+[ბგდვზთკლმნპჟრსტუფქღყშჩცძწჭხჯჰ])(?:ი)?-დან(?![ა-ჰ])/g, '$1იდან');

    // 3. Experiencer Dative Inversion: Nominative Copula Calque Eradication
    out = out.replace(/(?<![\u10A0-\u10FF])მე\s+ვარ\s+მშიერი(?![ა-ჰ])/g, 'მშია');
    out = out.replace(/(?<![\u10A0-\u10FF])ის\s+არის\s+მშიერი(?![ა-ჰ])/g, 'მას შია');
    out = out.replace(/(?<![\u10A0-\u10FF])მე\s+ვარ\s+მწყურვალი(?![ა-ჰ])/g, 'მწყურია');
    out = out.replace(/(?<![\u10A0-\u10FF])ის\s+არის\s+მწყურვალი(?![ა-ჰ])/g, 'მას სწყურია');
    ...
    const calques = [
        [/(?<![\u10A0-\u10FF])მიიღო\s+გადაწყვეტილება(?![ა-ჰ])/g, 'გადაწყვიტა'],
        [/(?<![\u10A0-\u10FF])მიიღეს\s+გადაწყვეტილება(?![ა-ჰ])/g, 'გადაწყვიტეს'],
        [/(?<![\u10A0-\u10FF])განახორციელა(?![ა-ჰ])/g, 'გააკეთა'],
        [/(?<![\u10A0-\u10FF])განხორციელება(?![ა-ჰ])/g, 'შესრულება'],
        [/(?<![\u10A0-\u10FF])ადგილი\s+ჰქონდა(?![ა-ჰ])/g, 'მოხდა'],
        [/(?<![\u10A0-\u10FF])წარმოადგენს(?![ა-ჰ])/g, 'არის'],
        [/(?<![\u10A0-\u10FF])მოცემულ\s+მომენტში(?![ა-ჰ])/g, 'ამჟამად'],
        [/(?<![\u10A0-\u10FF])გააკეთა\s+ღიმილი(?![ა-ჰ])/g, 'გაიღიმა'],
        [/(?<![\u10A0-\u10FF])ჰქონდა\s+საუბარი(?![ა-ჰ])/g, 'ისაუბრა'],
        [/(?<![\u10A0-\u10FF])ყურადღება\s+გადაიხადა(?![ა-ჰ])/g, 'ყურადღება მიაქცია'],
        [/(?<![\u10A0-\u10FF])მხედველობაში\s+მიიღო(?![ა-ჰ])/g, 'გაითვალისწინა'],
        [/(?<![\u10A0-\u10FF])გამოიყურებოდა\s+გარეგნულად(?![ა-ჰ])/g, 'მზერას გარეთ მიაპყრობდა'],
        [/(?<![\u10A0-\u10FF])თავის\s+გარეგნულ\s+ბიძგში(?![ა-ჰ])/g, 'თავის ამ გარეგან სწრაფვაში'],
        [/(?<![\u10A0-\u10FF])გარეგნობის\s+ზღვაში(?![ა-ჰ])/g, 'გარეგანი სამყაროს ოკეანეში'],
        [/(?<![\u10A0-\u10FF])გიმ(?:რ|კრ)ეკის\s+რელიგიები(?![ა-ჰ])/g, 'იაფფასიანი რელიგიები'],
        [/(?<![\u10A0-\u10FF])გიმ(?:რ|კრ)ეკის(?![ა-ჰ])/g, 'იაფფასიანი'],
        [/(?<![\u10A0-\u10FF])მან\s+ისინი\s+გაფრინდა(?![ა-ჰ])/g, 'კოსმოსში გატყორცნა ისინი'],
        [/(?<![\u10A0-\u10FF])მან\s+გაფრინდა(?![ა-ჰ])/g, 'ის გაფრინდა'],
        [/(?<![\u10A0-\u10FF])ქვებივით\s+აფრინდა\s+მათ(?![ა-ჰ])/g, 'ქვებივით ისროდა მათ'],
        [/(?<![\u10A0-\u10FF])ეს\s+უბედური\s+აგენტები\s+იპოვეს(?![ა-ჰ])/g, 'ამ უბედურმა აგენტებმა იპოვეს'],
        [/(?<![\u10A0-\u10FF])კაცობრიობა,\s*რომელიც\s+არ\s+იცის(?![ა-ჰ])/g, 'კაცობრიობა, რომელმაც არ იცის'],
        [/(?<![\u10A0-\u10FF])რომელიც\s+არ\s+იცის(?![ა-ჰ])/g, 'რომელმაც არ იცის'],
        [/(?<![\u10A0-\u10FF])საკმარისად\s+იყო\s+ნაპოვნი(?![ა-ჰ])/g, 'უხვად იყო ნაპოვნი'],
        [/(?<![\u10A0-\u10FF])უხვად\s+იყო\s+ნაპოვნი(?![ა-ჰ])/g, 'უხვად მოეპოვებოდათ'],
        [/(?<![\u10A0-\u10FF])უაზრობის\s+კოშმარი\s+უსასრულოდ(?![ა-ჰ])/g, 'უაზრობის უსასრულო კოშმარი'],
        [/(?<![\u10A0-\u10FF])თავსატეხების\s+ყუთებ(?:ი|ში|ს)(?![ა-ჰ])/g, 'შინაგან საიდუმლოებებში'],
        [/(?<![\u10A0-\u10FF])თავსატეხების\s+ყუთები\s+მათში(?![ა-ჰ])/g, 'მათში დაფარული თავსატეხები'],
        [/(?<![\u10A0-\u10FF])კაცობრიობის\s+გარეგნული\s+ბიძგი(?![ა-ჰ])/g, 'კაცობრიობის გარეგანი სწრაფვა'],
        [/(?<![\u10A0-\u10FF])იმ\s+ძველ\s+დღეებში(?![ა-ჰ])/g, 'იმ ძველ დროს'],
        [/(?<![\u10A0-\u10FF])უცოდინარი\s+ჭეშმარიტებების(?![ა-ჰ])/g, 'ჭეშმარიტების უცოდინარი'],
        [/(?<![\u10A0-\u10FF])ჭეშმარიტებების,\s*რომლებიც\s+დევს(?![ა-ჰ])/g, 'დაფარული ჭეშმარიტების'],
        [/(?<![\u10A0-\u10FF])რომლებიც\s+დევს\s+ყველა\s+ადამიანში(?![ა-ჰ])/g, 'რომელიც ყველა ადამიანშია დაფარული'],
        [/(?<![\u10A0-\u10FF])უბიძგებდა\s+მუდამ\s+გარეგნულად(?![ა-ჰ])/g, 'გამუდმებით გარეთ მიილტვოდა'],
        [/(?<![\u10A0-\u10FF])უგემოვნო\s+ზღვაში(?![ა-ჰ])/g, 'უგემურ ოკეანეში'],
        [/(?<![\u10A0-\u10FF])უფერო,\s*უგემოვნო,\s*უწონო(?![ა-ჰ])/g, 'უფერულ, უგემურ, უწონო'],
        [/(?<![\u10A0-\u10FF])(?:(?:კაცებსა|მამაკაცებსა)\s+და\s+ქალებს\s+)?(?:არ\s+ჰქონდათ|არ\s+ჰქონდა)\s+(?:მარტივი|ადვილი)\s+წვდომა(?![ა-ჰ])/g, 'ადამიანებს ხელი არ მიუწვდებოდათ'],
        [/(?<![\u10A0-\u10FF])ადამიანებს\s+არ\s+ჰქონდათ\s+მარტივი\s+წვდომა(?![ა-ჰ])/g, 'ადამიანებს ხელი არ მიუწვდებოდათ'],
        [/(?<![\u10A0-\u10FF])თავსატეხების\s+ყუთებ(?:ზე|ში|ი|ს)(?![ა-ჰ])/g, 'შინაგან საიდუმლოებებზე'],
        [/(?<![\u10A0-\u10FF])(?:ისინი\s+)?(?:გარეგნულად\s+გამოიყურებოდნენ|გამოიყურებოდნენ\s+გარეგნულად)(?![ა-ჰ])/g, 'მზერა გარეთ მიაპყრეს'],
        [/(?<![\u10A0-\u10FF])(?:ყოველთვის\s+)?გარედან\s+იძვრებოდნენ(?![ა-ჰ])/g, 'გამუდმებით გარეთ მიილტვოდნენ'],
        [/(?<![\u10A0-\u10FF])გარეგნობის\s+(?:უგემოვნო\s+)?ზღვ(?:ა|აში)(?![ა-ჰ])/g, 'გარეგანი სამყაროს უგემური ოკეანე'],
        [/(?<![\u10A0-\u10FF])გარე\s+სივრცე(?![ა-ჰ])/g, 'კოსმოსური სივრცე'],
        [/(?<![\u10A0-\u10FF])გარეგნულად\s+გახედვა(?![ა-ჰ])/g, 'მზერის გარეთ მიპყრობა'],
        [/(?<![\u10A0-\u10FF])თავიდან\s+ჭკვიანურად(?![ა-ჰ])/g, 'თავდაპირველად მკრთალად'],
        [/(?<![\u10A0-\u10FF])ჰაერიდან\s+ხომ\s+არ\s+გამოჩნდებოდნენ\?*(?![ა-ჰ])/g, 'პირდაპირ ჰაერში უნდა გამოკვეთილიყვნენ —'],
        [/(?<![\u10A0-\u10FF])საბოლოოდ\s+გახდა\s+ისეთივე\s+ხელშესახები(?![ა-ჰ])/g, 'ბოლოს კი ისეთივე ხელშესახებნი გამხდარიყვნენ'],
        [/(?<![\u10A0-\u10FF])ბრბო\s+არ\s+აპირებდა\s+მატერიალიზაციის\s+სანახავად(?![ა-ჰ])/g, 'ბრბოს მატერიალიზაციის ხილვა არ ეწერა'],
        [/(?<![\u10A0-\u10FF])თვალების\s+გასახარებლად(?![ა-ჰ])/g, 'თვალის დასატკბობად'],
        [/(?<![\u10A0-\u10FF])მათ\s+გარეთ\s+გაიხედეს(?![ა-ჰ])/g, 'მზერა გარეთ მიაპყრეს'],
        [/(?<![\u10A0-\u10FF])მათ\s+გაიხედეს(?![ა-ჰ])/g, 'გაიხედეს'],
        [/(?<![\u10A0-\u10FF])ისეთივე\s+არსებითი(?![ა-ჰ])/g, 'ისეთივე ხელშესახები'],
        [/(?<![\u10A0-\u10FF])ყველამ\s+იცის\s+როგორ(?![ა-ჰ])/g, 'ყველამ იცის, როგორ'],
        [/(?<![\u10A0-\u10FF])(იცის|იცოდა|იცოდნენ|ვიცი|ვიცით|გაიგო|გაიგეს|დაინახა|ნახა|მოუყვა|ჰკითხა|გაიგებს|გაახსენდა|მიხვდა|ახსოვს|ახსოვდა|გაარკვია)\s+(როგორ|რომ|რატომ|სად|როდის)(?![ა-ჰ])/g, '$1, $2'],
        // Somatic idioms
        [/(?<![\u10A0-\u10FF])(?:(?:მისი|თავისი)\s+)?სუნთქვა\s+დაიჭირა(?![ა-ჰ])/g, 'სული მოითქვა'],
        [/(?<![\u10A0-\u10FF])დაიჭირა\s+(?:(?:მისი|თავისი)\s+)?სუნთქვა(?![ა-ჰ])/g, 'სული მოითქვა'],
        [/(?<![\u10A0-\u10FF])(?:დაკარგა\s+სუნთქვა|სუნთქვა\s+დაკარგა)(?![ა-ჰ])/g, 'სუნთქვა შეეკრა'],
        [/(?<![\u10A0-\u10FF])მხრები\s+შეანჯღრია(?![ა-ჰ])/g, 'მხრები აიჩეჩა'],
        [/(?<![\u10A0-\u10FF])მხრების\s+შერყევა(?![ა-ჰ])/g, 'მხრების აჩეჩა'],
        [/(?<![\u10A0-\u10FF])თავი\s+შეანჯღრია(?![ა-ჰ])/g, 'თავი გააქნია'],
        [/(?<![\u10A0-\u10FF])სიცილში\s+აფეთქდა(?![ა-ჰ])/g, 'სიცილი წასკდა'],
        [/(?<![\u10A0-\u10FF])ცრემლებში\s+აფეთქდა(?![ა-ჰ])/g, 'ცრემლები წასკდა'],
        [/(?<![\u10A0-\u10FF])ყელი\s+გაიწმინდა(?![ა-ჰ])/g, 'ჩაახველა'],
        [/(?<![\u10A0-\u10FF])გული\s+ჩაუვარდა(?![ა-ჰ])/g, 'გული გადაუქანდა'],
        [/(?<![\u10A0-\u10FF])გულმა\s+დარტყმა\s+გამოტოვა(?![ა-ჰ])/g, 'გული შეუქანდა'],
        // Sci-Fi lexicon
        [/(?<![\u10A0-\u10FF])გარე\s+სივრცეში(?![ა-ჰ])/g, 'ღია კოსმოსში'],
        [/(?<![\u10A0-\u10FF])გარე\s+სივრცე(?![ა-ჰ])/g, 'კოსმოსური სივრცე'],
        [/(?<![\u10A0-\u10FF])ცარიელ(?:ი)?\s+სიცარიელეში(?![ა-ჰ])/g, 'უკიდეგანო სიცარიელეში'],
        [/(?<![\u10A0-\u10FF])ცარიელ(?:ი)?\s+სიცარიელეს(?![ა-ჰ])/g, 'უკიდეგანო სიცარიელეს'],
        [/(?<![\u10A0-\u10FF])ცარიელ(?:ი)?\s+სიცარიელედ(?![ა-ჰ])/g, 'უკიდეგანო სიცარიელედ'],
        [/(?<![\u10A0-\u10FF])ცარიელ(?:ი)?\s+სიცარიელე(?![ა-ჰ])/g, 'უკიდეგანო სიცარიელე'],
        [/(?<![\u10A0-\u10FF])თხელ(?:ი)?\s+ჰაერიდან\s+(?:გამოჩნდნენ|გამოჩნდებოდნენ)(?![ა-ჰ])/g, 'პირდაპირ ჰაერში გაჩნდნენ'],
        [/(?<![\u10A0-\u10FF])თხელ(?:ი)?\s+ჰაერიდან\s+გამოჩნდა(?![ა-ჰ])/g, 'პირდაპირ ჰაერში გაჩნდა'],
        [/(?<![\u10A0-\u10FF])თხელ(?:ი)?\s+ჰაერიდან\s+გამოჩენა(?![ა-ჰ])/g, 'პირდაპირ ჰაერში გაჩენა'],
        [/(?<![\u10A0-\u10FF])თხელ(?:ი)?\s+ჰაერიდან(?![ა-ჰ])/g, 'არაფრისგან'],
        [/(?<![\u10A0-\u10FF])(\b\d+|ათასი|მილიონი|რამდენიმე|მრავალი)\s+სინათლის\s+წლები(?![ა-ჰ])/g, '$1 სინათლის წელი'],
        [/(?<![\u10A0-\u10FF])(\b\d+|ორი|სამი|ათი|რამდენიმე)\s+კოსმოსური\s+გემები(?![ა-ჰ])/g, '$1 კოსმოსური გემი'],
        // Animacy possession concord
        [/(?<![\u10A0-\u10FF])(მას|ვის|ყველას)\s+ჰყავს\s+(მანქანა|სახლი|წიგნი|ტელეფონი|კალამი|ბინა|ქონება)(?![ა-ჰ])/g, '$1 აქვს $2'],
        [/(?<![\u10A0-\u10FF])(მას|ვის|ყველას)\s+ჰყავდა\s+(მანქანა|სახლი|წიგნი|ტელეფონი|კალამი|ბინა|ქონება)(?![ა-ჰ])/g, '$1 ჰქონდა $2'],
        [/(?<![\u10A0-\u10FF])(მას|ვის|ყველას)\s+აქვს\s+(ძაღლი|კატა|ცხენი|შვილი|მეგობარი|ძმა|და|ვაჟი|ქალიშვილი|მტერი|მრჩეველი|მოწაფე|მასწავლებელი)(?![ა-ჰ])/g, '$1 ჰყავს $2'],
        [/(?<![\u10A0-\u10FF])(მას|ვის|ყველას)\s+ჰქონდა\s+(ძაღლი|კატა|ცხენი|შვილი|მეგობარი|ძმა|და|ვაჟი|ქალიშვილი|მტერი|მრჩეველი|მოწაფე|მასწავლებელი)(?![ა-ჰ])/g, '$1 ჰყავდა $2'],
        [/(?<![\u10A0-\u10FF])არ\s+აქვს\s+(ძაღლი|კატა|ცხენი|შვილი|მეგობარი|ძმა|და|ვაჟი|ქალიშვილი|მტერი)(?![ა-ჰ])/g, 'არ ჰყავს $1'],
        [/(?<![\u10A0-\u10FF])არ\s+ჰქონდა\s+(ძაღლი|კატა|ცხენი|შვილი|მეგობარი|ძმა|და|ვაჟი|ქალიშვილი|მტერი)(?![ა-ჰ])/g, 'არ ჰყავდა $1'],
        // Temporal compacting
        [/(?<![\u10A0-\u10FF])საათების\s+განმავლობაში(?![ა-ჰ])/g, 'საათობით'],
        [/(?<![\u10A0-\u10FF])დიდი\s+დროის\s+განმავლობაში(?![ა-ჰ])/g, 'დიდხანს'],
        [/(?<![\u10A0-\u10FF])დღეების\s+(?:განმავლობაში|მანძილზე)(?![ა-ჰ])/g, 'დღეობით'],
        [/(?<![\u10A0-\u10FF])წლების\s+(?:განმავლობაში|მანძილზე)(?![ა-ჰ])/g, 'წლობით'],
        [/(?<![\u10A0-\u10FF])ერთი\s+მომენტისთვის(?![ა-ჰ])/g, 'წამით'],
        [/(?<![\u10A0-\u10FF])ერთი\s+წამის\s+განმავლობაში(?![ა-ჰ])/g, 'ერთი წამით'],
        [/(?<![\u10A0-\u10FF])დღიდან\s+დღემდე(?![ა-ჰ])/g, 'დღითი დღე'],
        [/(?<![\u10A0-\u10FF])დროიდან\s+დრომდე(?![ა-ჰ])/g, 'დროდადრო'],
        [/(?<![\u10A0-\u10FF])ბოლო\s+დროის\s+განმავლობაში(?![ა-ჰ])/g, 'ბოლო დროს'],
        [/(?<![\u10A0-\u10FF])იმ\s+მომენტის\s+განმავლობაში(?![ა-ჰ])/g, 'იმ მომენტში'],
        // Indirect object & light verbs
        [/(?<![\u10A0-\u10FF])გააკეთა\s+აზრი(?![ა-ჰ])/g, 'აზრი შეიძინა'],
        [/(?<![\u10A0-\u10FF])არ\s+აკეთებს\s+აზრს(?![ა-ჰ])/g, 'აზრს მოკლებულია'],
        [/(?<![\u10A0-\u10FF])ყურადღება\s+გადაიხადა(?![ა-ჰ])/g, 'ყურადღება მიაქცია'],
        [/(?<![\u10A0-\u10FF])ადგილი\s+აიღო(?![ა-ჰ])/g, 'ჩატარდა'],
        [/(?<![\u10A0-\u10FF])კითხა\s+მას(?![ა-ჰ])/g, 'ჰკითხა მას'],
        [/(?<![\u10A0-\u10FF])—\s*კითხა\s+მან(?![ა-ჰ])/g, '— ჰკითხა მან'],
        // Psychological & Mental State Collocations
        [/(?<![\u10A0-\u10FF])(?:(?:მისი|თავისი)\s+)?გონება\s+დაკარგა(?![ა-ჰ])/g, 'ჭკუიდან შეიშალა'],
        [/(?<![\u10A0-\u10FF])დაკარგა\s+(?:(?:მისი|თავისი)\s+)?გონება(?![ა-ჰ])/g, 'ჭკუიდან შეიშალა'],
        [/(?<![\u10A0-\u10FF])(?:გააკეთა|შეადგინა)\s+(?:(?:მისი|თავისი)\s+)?გონება(?!\s+ერთხელ\s+და\s+ყველასთვის)(?![ა-ჰ])/g, 'გადაწყვიტა'],
        [/(?<![\u10A0-\u10FF])შეცვალა\s+(?:(?:მისი|თავისი)\s+)?გონება(?![ა-ჰ])/g, 'გადაიფიქრა'],
        [/(?<![\u10A0-\u10FF])შეინახა\s+გონებაში(?![ა-ჰ])/g, 'გაითვალისწინა'],
        [/(?<![\u10A0-\u10FF])შეინახეთ\s+გონებაში(?![ა-ჰ])/g, 'გაითვალისწინეთ'],
        [/(?<![\u10A0-\u10FF])შეინახე\s+გონებაში(?![ა-ჰ])/g, 'გაითვალისწინე'],
        [/(?<![\u10A0-\u10FF])გადაკვეთა\s+(?:მისი|მისმა|მის)\s+გონებ(?:ა|ამ|ას)?(?![ა-ჰ])/g, 'აზრად მოუვიდა'],
        [/(?<![\u10A0-\u10FF])(?:მისი|მისმა|მის)\s+გონებ(?:ა|ამ|ას)?\s+გადაკვეთა(?![ა-ჰ])/g, 'აზრად მოუვიდა'],
        [/(?<![\u10A0-\u10FF])გაიარა\s+მის\s+გონებაში(?![ა-ჰ])/g, 'აზრად მოუვიდა'],
        [/(?<![\u10A0-\u10FF])მისი\s+გონების\s+უკან(?![ა-ჰ])/g, 'გულის სიღრმეში'],
        [/(?<![\u10A0-\u10FF])გონებიდან\s+გადავიდა(?![ა-ჰ])/g, 'ჭკუიდან შეიშალა'],
        // Sensory, Visual & Gaze Collocations
        [/(?<![\u10A0-\u10FF])დაიჭირა\s+(?:(?:მისი|თავისი)\s+)?თვალ(?:ი)?(?![ა-ჰ])/g, 'თვალი მოჰკრა'],
        [/(?<![\u10A0-\u10FF])თვალ(?:ი)?\s+დაიჭირა(?![ა-ჰ])/g, 'თვალი მოჰკრა'],
        [/(?<![\u10A0-\u10FF])დაადო\s+თვალი\s+(?:მას|მასზე)(?![ა-ჰ])/g, 'თვალი შეავლო მას'],
        [/(?<![\u10A0-\u10FF])დადო\s+თვალები(?![ა-ჰ])/g, 'თვალი შეავლო'],
        [/(?<![\u10A0-\u10FF])შეინახა\s+თვალი(?![ა-ჰ])/g, 'თვალყური ადევნა'],
        [/(?<![\u10A0-\u10FF])შეინახეთ\s+თვალი(?![ა-ჰ])/g, 'თვალყური ადევნეთ'],
        [/(?<![\u10A0-\u10FF])შეინახე\s+თვალი(?![ა-ჰ])/g, 'თვალყური ადევნე'],
        [/(?<![\u10A0-\u10FF])(?:თვალის\s+ერთ\s+დახამხამებაში|ერთი\s+თვალის\s+დახამხამებაში)(?![ა-ჰ])/g, 'თვალის დახამხამებაში'],
        [/(?<![\u10A0-\u10FF])თვალები\s+დახუჭა\s+ამაზე(?![ა-ჰ])/g, 'თვალი დახუჭა ამაზე'],
        // Instrumental Bodily Organ Singular Concord
        [/(?<![\u10A0-\u10FF])(?:(?:თავისი|მის)\s+)?საკუთარი\s+თვალებით(?![ა-ჰ])/g, 'საკუთარი თვალით'],
        [/(?<![\u10A0-\u10FF])(?:(?:თავისი|მის)\s+)?საკუთარი\s+ყურებით(?![ა-ჰ])/g, 'საკუთარი ყურით'],
        [/(?<![\u10A0-\u10FF])შიშველი\s+ხელებით(?![ა-ჰ])/g, 'შიშველი ხელით'],
        [/(?<![\u10A0-\u10FF])ფეხებზე\s+იარა(?![ა-ჰ])/g, 'ფეხით იარა'],
        [/(?<![\u10A0-\u10FF])ფეხებით\s+იარა(?![ა-ჰ])/g, 'ფეხით იარა'],
        [/(?<![\u10A0-\u10FF])თავისი\s+ხელებით\s+გააკეთა(?![ა-ჰ])/g, 'საკუთარი ხელით გააკეთა'],
        [/(?<![\u10A0-\u10FF])ღია\s+ხელებით\s+მიიღ(?:ო|ეს)(?![ა-ჰ])/g, 'გულღიად მიიღო'],
        // Spatial Dynamics & Binomial Motion Synthesis
        [/(?<![\u10A0-\u10FF])გვერდი\s+გვერდით(?![ა-ჰ])/g, 'მხარდამხარ'],
        [/(?<![\u10A0-\u10FF])უკან\s+და\s+წინ(?![ა-ჰ])/g, 'წინ და უკან'],
        [/(?<![\u10A0-\u10FF])სახე\s+სახესთან(?![ა-ჰ])/g, 'პირისპირ'],
        [/(?<![\u10A0-\u10FF])(?:არსად\s+შუაში|არაფრის\s+შუაგულში)(?![ა-ჰ])/g, 'უკაცრიელ ადგილას'],
        [/(?<![\u10A0-\u10FF])(?:მთელ\s+ადგილზე|ყველა\s+ადგილზე)(?![ა-ჰ])/g, 'ყველგან'],
        [/(?<![\u10A0-\u10FF])ნაბიჯი\s+ნაბიჯით(?![ა-ჰ])/g, 'ნაბიჯ-ნაბიჯ'],
        // Narrative Discourse Connectors & Evidential Transitionals
        [/(?<![\u10A0-\u10FF])(?:პირველ\s+შეხედვაზე|პირველ\s+მზერაზე)(?![ა-ჰ])/g, 'ერთი შეხედვით'],
        [/(?<![\u10A0-\u10FF])როგორც\s+ფაქტის\s+საკითხი(?![ა-ჰ])/g, 'სინამდვილეში'],
        [/(?<![\u10A0-\u10FF])ყველა\s+მოულოდნელად(?![ა-ჰ])/g, 'უეცრად'],
        [/(?<![\u10A0-\u10FF])უფრო\s+ადრე\s+თუ\s+უფრო\s+გვიან(?![ა-ჰ])/g, 'ადრე თუ გვიან'],
        [/(?<![\u10A0-\u10FF])თავიდან\s+ფეხის\s+თითამდე(?![ა-ჰ])/g, 'თავით ფეხამდე'],
        [/(?<![\u10A0-\u10FF])დროის\s+დასაწყისიდან(?![ა-ჰ])/g, 'ოდითგანვე'],
        // Verba Dicendi & Dialogue Inquit Synthesis
        [/(?<![\u10A0-\u10FF])ჩურჩულით\s+თქვა(?![ა-ჰ])/g, 'ჩაიჩურჩულა'],
        [/(?<![\u10A0-\u10FF])ყვირილით\s+თქვა(?![ა-ჰ])/g, 'დაიყვირა'],
        [/(?<![\u10A0-\u10FF])მისცა\s+პასუხი(?![ა-ჰ])/g, 'უპასუხა'],
        [/(?<![\u10A0-\u10FF])გააკეთა\s+კომენტარი(?![ა-ჰ])/g, 'აღნიშნა'],
        [/(?<![\u10A0-\u10FF])დასვა\s+(?:შე)?კითხვა(?![ა-ჰ])/g, 'ჰკითხა'],
        [/(?<![\u10A0-\u10FF])(?:იკითხა\s+უკან|უკან\s+იკითხა)(?![ა-ჰ])/g, 'შეუბრუნა კითხვა'],
        // Involuntary Actions & Somatic Reflexes
        [/(?<![\u10A0-\u10FF])(?:გამოუშვა|ამოუშვა)\s+ოხვრა(?![ა-ჰ])/g, 'ამოიოხრა'],
        [/(?<![\u10A0-\u10FF])(?:მისი\s+)?გული\s+ჩაიძირა(?![ა-ჰ])/g, 'გული გადაუქანდა'],
        [/(?<![\u10A0-\u10FF])ჟრუანტელმა\s+გაიარა\s+მის\s+ხერხემალში(?![ა-ჰ])/g, 'ტანში ჟრუანტელმა დაუარა'],
        [/(?<![\u10A0-\u10FF])(?:აიღო\s+ღრმა\s+სუნთქვა|ღრმა\s+სუნთქვა\s+აიღო)(?![ა-ჰ])/g, 'ღრმად ჩაისუნთქა'],
        [/(?<![\u10A0-\u10FF])შეინახა\s+სუნთქვა(?![ა-ჰ])/g, 'სუნთქვა შეიკრა'],
        // Epistemic Modals & Evidential Stance
        [/(?<![\u10A0-\u10FF])მიდის\s+უთქმელად(?![ა-ჰ])/g, 'თავისთავად ცხადია'],
        [/(?<![\u10A0-\u10FF])ყველა\s+ალბათობაში(?![ა-ჰ])/g, 'დიდი ალბათობით'],
        [/(?<![\u10A0-\u10FF])(?:რომ\s+თქვა\s+სიმართლე|სიმართლე\s+რომ\s+თქვა)(?![ა-ჰ])/g, 'სიმართლე რომ ითქვას'],
        [/(?<![\u10A0-\u10FF])ეჭვის\s+(?:ყოველგვარი\s+)?ჩრდილის\s+გარეშე(?![ა-ჰ])/g, 'ყოველგვარი ეჭვის გარეშე'],
        [/(?<![\u10A0-\u10FF])არ\s+არის\s+ეჭვი,\s*რომ(?![ა-ჰ])/g, 'ეჭვგარეშეა, რომ'],
        // Temporal Duratives & Inceptives
        [/(?<![\u10A0-\u10FF])(?:მთელი\s+დღე\s+გრძელი|მთელი\s+დღის\s+გასწვრივ)(?![ა-ჰ])/g, 'მთელი დღის განმავლობაში'],
        [/(?<![\u10A0-\u10FF])დროის\s+კურსში(?![ა-ჰ])/g, 'დროთა განმავლობაში'],
        [/(?<![\u10A0-\u10FF])დროიდან\s+დროში(?![ა-ჰ])/g, 'დროდადრო'],
        // Adversative & Concessive Antithesis
        [/(?<![\u10A0-\u10FF])საპირისპიროზე(?![ა-ჰ])/g, 'პირიქით'],
        [/(?<![\u10A0-\u10FF])ერთ\s+ხელზე(?![ა-ჰ])/g, 'ერთი მხრივ'],
        [/(?<![\u10A0-\u10FF])მეორე\s+ხელზე(?![ა-ჰ])/g, 'მეორე მხრივ'],
        [/(?<![\u10A0-\u10FF])თანაბრად\s+ასე(?![ა-ჰ])/g, 'მიუხედავად ამისა'],
        [/(?<![\u10A0-\u10FF])ყველა\s+უფრო(?![ა-ჰ])/g, 'მით უმეტეს'],
        [/(?<![\u10A0-\u10FF])მეტი\s+თუ\s+ნაკლები(?![ა-ჰ])/g, 'მეტ-ნაკლებად'],
        [/(?<![\u10A0-\u10FF])შორს\s+მისგან(?![ა-ჰ])/g, 'სრულებითაც არა'],
        // Auditory Verbs & Sound Symbolism
        [/(?<![\u10A0-\u10FF])კარი\s+(?:დაეჯახა\s+დახურულად|დაეჯახა)(?![ა-ჰ])/g, 'კარი გაჯახუნდა'],
        [/(?<![\u10A0-\u10FF])იატაკი\s+(?:ყვიროდა|გამოსცემდა\s+ხმას)(?![ა-ჰ])/g, 'იატაკი აჭრაჭუნდა'],
        [/(?<![\u10A0-\u10FF])(?:მისი\s+)?კბილები\s+(?:ლაპარაკობდნენ|აკაკუნებდნენ)(?![ა-ჰ])/g, 'კბილი კბილზე ადიოდა'],
        [/(?<![\u10A0-\u10FF])ნაბიჯები\s+ექოდ\s+ისმოდა(?![ა-ჰ])/g, 'ნაბიჯების ხმა ექოდ გაისმა'],
        [/(?<![\u10A0-\u10FF])დააკაკუნა\s+კარებზე(?![ა-ჰ])/g, 'კარზე დააკაკუნა'],
        // Emotional & Cognitive Metaphors
        [/(?<![\u10A0-\u10FF])(?:დაკარგა\s+ტემპერამენტი|ტემპერამენტი\s+დაკარგა)(?![ა-ჰ])/g, 'მოთმინება დაკარგა'],
        [/(?<![\u10A0-\u10FF])შეინარჩუნა\s+სიგრილე(?![ა-ჰ])/g, 'სიმშვიდე შეინარჩუნა'],
        [/(?<![\u10A0-\u10FF])დაჯდა\s+მის\s+ნერვებზე(?![ა-ჰ])/g, 'ნერვებს უშლიდა'],
        [/(?<![\u10A0-\u10FF])გული\s+დადო\s+ამაზე(?![ა-ჰ])/g, 'გულით მოინდომა'],
        [/(?<![\u10A0-\u10FF])გული\s+გატეხა(?![ა-ჰ])/g, 'გული მოუკლა'],
        // Physical Posture & Stative Orientation
        [/(?<![\u10A0-\u10FF])იჯდა\s+გადაჯვარედინებული\s+ფეხებით(?![ა-ჰ])/g, 'ფეხმორთხმით იჯდა'],
        [/(?<![\u10A0-\u10FF])იდგა\s+ფეხის\s+თითებზე(?![ა-ჰ])/g, 'ფეხის წვერებზე იდგა'],
        [/(?<![\u10A0-\u10FF])დაეცა\s+(?:მის\s+)?მუხლებზე(?![ა-ჰ])/g, 'მუხლებზე დაემხო'],
        [/(?<![\u10A0-\u10FF])გადააჯვარედინა\s+(?:თავისი\s+)?ხელები(?![ა-ჰ])/g, 'გულზე ხელები დაიკრიფა'],
        [/(?<![\u10A0-\u10FF])დახარა\s+(?:თავისი\s+)?თავი(?![ა-ჰ])/g, 'თავი დახარა'],
        // Quantificational Intensifiers & Degree Particles
        [/(?<![\u10A0-\u10FF])დიდით\s+და\s+ვრცელით(?![ა-ჰ])/g, 'მთლიანობაში'],
        [/(?<![\u10A0-\u10FF])ერთხელ\s+ლურჯ\s+მთვარეზე(?![ა-ჰ])/g, 'ძალზე იშვიათად'],
        [/(?<![\u10A0-\u10FF])გასწვრივ\s+და\s+გასწვრივ(?![ა-ჰ])/g, 'თავიდან ბოლომდე'],
        [/(?<![\u10A0-\u10FF])უსაფრთხოდ\s+და\s+ხმით(?![ა-ჰ])/g, 'ცოცხალ-ჯანსაღი'],
        [/(?<![\u10A0-\u10FF])ნელა,\s*მაგრამ\s+დარწმუნებით(?![ა-ჰ])/g, 'ნელა, მაგრამ შეუპოვრად'],
        // Polysemous Motion Collocations & Phrasal Directionals
        [/(?<![\u10A0-\u10FF])მობრუნდა\s+უკან(?![ა-ჰ])/g, 'გამობრუნდა'],
        [/(?<![\u10A0-\u10FF])მოშორებით\s+შებრუნდა(?![ა-ჰ])/g, 'ზურგი შეაქცია'],
        [/(?<![\u10A0-\u10FF])მოშორებით\s+შეხედა(?![ა-ჰ])/g, 'მზერა აარიდა'],
        [/(?<![\u10A0-\u10FF])მზერა\s+ესროლა(?![ა-ჰ])/g, 'თვალი შეავლო'],
        [/(?<![\u10A0-\u10FF])მხედველობა\s+დაიჭირა(?![ა-ჰ])/g, 'თვალი მოჰკრა'],
        // Environmental & Meteorological Verbs
        [/(?<![\u10A0-\u10FF])(?:წვიმამ\s+დაიწყო\s+მძიმედ\s+წვიმა|წვიმა\s+წამოვიდა\s+მძიმედ)(?![ა-ჰ])/g, 'კოკისპირულად გაწვიმდა'],
        [/(?<![\u10A0-\u10FF])ჭექა-ქუხილმა\s+გააგორა\s+მთებზე(?![ა-ჰ])/g, 'მთებში ქუხილმა დაიგრგვინა'],
        [/(?<![\u10A0-\u10FF])ნისლი\s+დაჯდა\s+ხეობაზე(?![ა-ჰ])/g, 'ხეობას ნისლი ჩამოწვა'],
        [/(?<![\u10A0-\u10FF])ქარი\s+ყვიროდა\s+ხეებში(?![ა-ჰ])/g, 'ქარი ხეებში ღმუოდა'],
        [/(?<![\u10A0-\u10FF])მზე\s+სცემდა\s+დაუნდობლად(?![ა-ჰ])/g, 'მზე დაუნდობლად აჭერდა'],
        // Phrasal Resultatives & State-Change Aspectuals
        [/(?<![\u10A0-\u10FF])(?:ცეცხლმა\s+თავი\s+დაწვა|ცეცხლი\s+დაიწვა\s+თვითონ)(?![ა-ჰ])/g, 'ცეცხლი ჩაქრა'],
        [/(?<![\u10A0-\u10FF])მდინარე\s+გაიქცა\s+მშრალი(?![ა-ჰ])/g, 'მდინარე დაშრა'],
        [/(?<![\u10A0-\u10FF])საათმა\s+ტიკტიკით\s+გაუშვა\s+წუთები(?![ა-ჰ])/g, 'საათი წუთებს ითვლიდა'],
        [/(?<![\u10A0-\u10FF])შუშა\s+დაიმსხვრა\s+ნაჭრებში(?![ა-ჰ])/g, 'შუშა ნამსხვრევებად იქცა'],
        [/(?<![\u10A0-\u10FF])ხმა\s+გაფერმკრთალდა\s+სიჩუმეში(?![ა-ჰ])/g, 'ხმა თანდათან მიწყდა'],
        // Psychological Volition & Dispositional Adjectives
        [/(?<![\u10A0-\u10FF])(?:(?:იყო\s+)?სიტყვების\s+(?:დანაკარგში|დაკარგვაში)(?:\s+იყო)?)(?![ა-ჰ])/g, 'სიტყვა ვეღარ მოეძებნა'],
        [/(?<![\u10A0-\u10FF])(?:(?:შეადგინა\s+თავისი\s+გონება|გადაწყვიტა)\s+ერთხელ\s+და\s+ყველასთვის|გონება\s+ერთხელ\s+და\s+ყველასთვის)(?![ა-ჰ])/g, 'საბოლოოდ გადაწყვიტა'],
        [/(?<![\u10A0-\u10FF])ჰქონდა\s+ცუდი\s+გრძნობა\s+ამაზე(?![ა-ჰ])/g, 'ცუდი წინათგრძნობა ჰქონდა'],
        [/(?<![\u10A0-\u10FF])(?:ეს\s+გულთან\s+აიღო|აიღო\s+ეს\s+გულში)(?![ა-ჰ])/g, 'გულთან ახლოს მიიტანა'],
        [/(?<![\u10A0-\u10FF])თავის\s+გვერდით\s+იყო\s+სიბრაზით(?![ა-ჰ])/g, 'განრისხებისგან ჭკუაზე აღარ იყო'],
        // Cognitive Perception & Realization Idioms
        [/(?<![\u10A0-\u10FF])ეს\s+გათენდა\s+მასზე,\s*რომ(?![ა-ჰ])/g, 'უეცრად მიხვდა, რომ'],
        [/(?<![\u10A0-\u10FF])მოვიდა\s+თავის\s+გრძნობებში(?![ა-ჰ])/g, 'გონს მოეგო'],
        [/(?<![\u10A0-\u10FF])დაინახა\s+მოტყუების\s+გავლით(?![ა-ჰ])/g, 'სიცრუე მაშინვე ამოიცნო'],
        [/(?<![\u10A0-\u10FF])მიიღო\s+გარანტირებულად(?![ა-ჰ])/g, 'თავისთავად ცხადად მიიჩნია'],
        [/(?<![\u10A0-\u10FF])დაკარგა\s+დროის\s+კვალი(?![ა-ჰ])/g, 'დროის შეგრძნება დაკარგა'],
        // Collocational Intensifiers & Bound Spatial Preverbs
        [/(?<![\u10A0-\u10FF])ღრმად\s+ფიქრში(?![ა-ჰ])/g, 'ფიქრებში ჩაძირული'],
        [/(?<![\u10A0-\u10FF])ლურჯიდან\s+გარეთ(?![ა-ჰ])/g, 'მოულოდნელად'],
        [/(?<![\u10A0-\u10FF])ყველა\s+შანსების\s+წინააღმდეგ(?![ა-ჰ])/g, 'ყოველგვარი დაბრკოლების მიუხედავად'],
        [/(?<![\u10A0-\u10FF])ეჭვის\s+ჩრდილის\s+მიღმა(?![ა-ჰ])/g, 'ეჭვგარეშეა'],
        [/(?<![\u10A0-\u10FF])ზემოდან\s+ქვემომდე(?![ა-ჰ])/g, 'თავიდან ბოლომდე'],
        // Sensory Perception & Somatic Collocations
        [/(?<![\u10A0-\u10FF])(?:მძაფრი\s+სუნი\s+ეკიდა|სუნი\s+ეკიდა\s+ჰაერში)(?![ა-ჰ])/g, 'მძაფრი სუნი იდგა'],
        [/(?<![\u10A0-\u10FF])სიცივემ\s+გაიარა\s+მის\s+ხერხემალში(?![ა-ჰ])/g, 'ტანში ცივმა ჟრუანტელმა დაუარა'],
        [/(?<![\u10A0-\u10FF])ზარი\s+მის\s+ყურებში(?![ა-ჰ])/g, 'ყურებში წუილი'],
        [/(?<![\u10A0-\u10FF])ყურებში\s+რეკვა\s+დაიწყო(?![ა-ჰ])/g, 'ყურები აუწუილდა'],
        [/(?<![\u10A0-\u10FF])(?:დატოვა\s+მწარე\s+გემო\s+პირში|დატოვა\s+მწარე\s+გემო\s+მის\s+პირში|დატოვა\s+მწარე\s+გემო)(?![ა-ჰ])/g, 'მწარე გემო დაუტოვა'],
        [/(?<![\u10A0-\u10FF])სიცივემ\s+გაჭრა\s+ძვალამდე(?![ა-ჰ])/g, 'სიცივემ ძვლებამდე გაატანა'],
        // Epistemic Evidentiality & Discourse Pragmatics
        [/(?<![\u10A0-\u10FF])როგორც\s+ფაქტის\s+საკითხი(?![ა-ჰ])/g, 'სინამდვილეში'],
        [/(?<![\u10A0-\u10FF])ყველა\s+ანგარიშით(?![ა-ჰ])/g, 'როგორც ჩანს'],
        [/(?<![\u10A0-\u10FF])არ\s+აქვს\s+მნიშვნელობა\s+რა\s+მოხდება(?![ა-ჰ])/g, 'რაც არ უნდა მოხდეს'],
        [/(?<![\u10A0-\u10FF])სხვა\s+სიტყვებში(?![ა-ჰ])/g, 'სხვა სიტყვებით რომ ვთქვათ'],
        [/(?<![\u10A0-\u10FF])სიმართლე\s+სათქმელად(?![ა-ჰ])/g, 'სიმართლე რომ ითქვას'],
        // Dynamic Motion & Spatial Transit Idioms
        [/(?<![\u10A0-\u10FF])(?:გაიქცა\s+უკან\s+ყურების\s+გარეშე|უკან\s+ყურების\s+გარეშე\s+გაიქცა)(?![ა-ჰ])/g, 'უკანმოუხედავად გაიქცა'],
        [/(?<![\u10A0-\u10FF])(?:გააკეთა\s+თავისი\s+გზა|თავისი\s+გზა\s+გააკეთა)(?![ა-ჰ])/g, 'გზა გაიკვლია'],
        [/(?<![\u10A0-\u10FF])მიჰყვა\s+ცხელ\s+ქუსლებზე(?![ა-ჰ])/g, 'ფეხდაფეხ მიჰყვებოდა'],
        [/(?<![\u10A0-\u10FF])გაიარა\s+გვერდით\s+გარშემო(?![ა-ჰ])/g, 'გვერდი აუარა'],
        [/(?<![\u10A0-\u10FF])მიჰყვა\s+თავისი\s+თვალებით(?![ა-ჰ])/g, 'თვალი გააყოლა'],
        // Conversational Interlocution & Dialogue Discourse
        [/(?<![\u10A0-\u10FF])(?:გაჭრა\s+მისი\s+სიტყვა|შეაწყვეტინა\s+იგი)(?![ა-ჰ])/g, 'სიტყვა შეაწყვეტინა'],
        [/(?<![\u10A0-\u10FF])დაარტყა\s+საუბარი(?![ა-ჰ])/g, 'საუბარი გააბა'],
        [/(?<![\u10A0-\u10FF])ილაპარაკა\s+უკან(?![ა-ჰ])/g, 'სიტყვა შეუბრუნა'],
        [/(?<![\u10A0-\u10FF])გამოხატა\s+აზრი(?![ა-ჰ])/g, 'აზრი გამოთქვა'],
        [/(?<![\u10A0-\u10FF])ილაპარაკა\s+თავისი\s+გონება(?![ა-ჰ])/g, 'გულახდილად გამოთქვა აზრი'],
        // Existential State Transitions & Temporal Duratives
        [/(?<![\u10A0-\u10FF])დრო\s+გადიოდა\s+გარეთ(?![ა-ჰ])/g, 'დრო მიდიოდა'],
        [/(?<![\u10A0-\u10FF])დღის\s+გავლით(?![ა-ჰ])/g, 'მთელი დღის განმავლობაში'],
        [/(?<![\u10A0-\u10FF])(?:გაიყინა\s+ადგილზე\s+ერთ\s+წამში|გაიყინა\s+ადგილზე)(?![ა-ჰ])/g, 'წამიერად გაქვავდა'],
        [/(?<![\u10A0-\u10FF])დაიჭირა\s+მომენტი(?![ა-ჰ])/g, 'დრო იხელთა'],
        [/(?<![\u10A0-\u10FF])(?:ძლივს\s+(?:დაიჭირა\s+(?:(?:მისი|თავისი)\s+)?სუნთქვა|სული\s+მოითქვა)|ძლივს\s+დაიჭირა\s+თავისი\s+სუნთქვა)(?![ა-ჰ])/g, 'სულს ძლივს ითქვამდა']
    ];
    ...
    const postpositionalFixes = [
        ...
        [/(?<!მოვიდა\s)(?<!შემოვიდა\s)(?<![\u10A0-\u10FF])შიგნით\s+ოთახში(?![ა-ჰ])/g, 'ოთახში'],
    ...
    // 47. Somatic & Bodily Reaction Idioms (სომატური და ფიზიოლოგიური რეაქციების იდიომები)
    const somaticBodilyFixes = [
        [/(?<![\u10A0-\u10FF])(?:(?:მისი|თავისი)\s+)?სუნთქვა\s+დაიჭირა(?![ა-ჰ])/g, 'სული მოითქვა'],
        [/(?<![\u10A0-\u10FF])დაიჭირა\s+(?:(?:მისი|თავისი)\s+)?სუნთქვა(?![ა-ჰ])/g, 'სული მოითქვა'],
        [/(?<![\u10A0-\u10FF])დაკარგა\s+სუნთქვა(?![ა-ჰ])/g, 'სუნთქვა შეეკრა'],
        [/(?<![\u10A0-\u10FF])სუნთქვა\s+დაკარგა(?![ა-ჰ])/g, 'სუნთქვა შეეკრა'],
        [/(?<![\u10A0-\u10FF])მხრები\s+შეანჯღრია(?![ა-ჰ])/g, 'მხრები აიჩეჩა'],
        [/(?<![\u10A0-\u10FF])ყელი\s+გაიწმინდა(?![ა-ჰ])/g, 'ჩაახველა'],
        [/(?<![\u10A0-\u10FF])სიცილში\s+აფეთქდა(?![ა-ჰ])/g, 'სიცილი წასკდა'],
        [/(?<![\u10A0-\u10FF])ცრემლებში\s+აფეთქდა(?![ა-ჰ])/g, 'ცრემლები წასკდა'],
    ...
    for (const [re, repl] of somaticBodilyFixes) {
        out = out.replace(re, repl);
    }

    // 48. Speculative & Sci-Fi Collocations (კოსმოსური და ფანტასტიკური პროზის ლექსიკა)
    const sciFiCollocationFixes = [
        [/(?<![\u10A0-\u10FF])გარე\s+სივრც(?:ე|ეში|იდან)(?![ა-ჰ])/g, 'კოსმოსურ სივრცეში'],
        [/(?<![\u10A0-\u10FF])ცარიელ(?:ი)?\s+სიცარიელ(?:ე|ეში|იდან)(?![ა-ჰ])/g, 'უკიდეგანო სიცარიელეში'],
        [/(?<![\u10A0-\u10FF])თხელ(?:ი)?\s+ჰაერიდან(?![ა-ჰ])/g, 'არაფრისგან'],
        [/(?<![\u10A0-\u10FF])გარეთ\s+ყურება(?![ა-ჰ])/g, 'გარეთ მზერა'],
        [/(?<![\u10A0-\u10FF])გარედან\s+იძვრებოდნენ(?![ა-ჰ])/g, 'გარეთ მიიწევდნენ'],
    ];
    for (const [re, repl] of sciFiCollocationFixes) {
        out = out.replace(re, repl);
    }

    // 49. Polypersonal Possession & Animacy Concord (ქონა/ყოლა ანიმაციურობის მიხედვით)
    const animacyPossessionFixes = [
        [/(?<![\u10A0-\u10FF])(მას\s+)?აქვს\s+(ძაღლი|კატა|ცხენი|შვილი|მეგობარი|ძმა|და|ვაჟი|ქალიშვილი)(?![ა-ჰ])/g, '$1ჰყავს $2'],
        [/(?<![\u10A0-\u10FF])(მას\s+)?ჰქონდა\s+(ძაღლი|კატა|ცხენი|შვილი|მეგობარი|ძმა|და|ვაჟი|ქალიშვილი)(?![ა-ჰ])/g, '$1ჰყავდა $2'],
        [/(?<![\u10A0-\u10FF])(მას\s+)?არ\s+აქვს\s+(ძაღლი|კატა|ცხენი|შვილი|მეგობარი|ძმა|და|ვაჟი|ქალიშვილი)(?![ა-ჰ])/g, '$1არ ჰყავს $2'],
    ];
    for (const [re, repl] of animacyPossessionFixes) {
        out = out.replace(re, repl);
    }

    // 50. Temporal Adverbial Compacting (დროითი ზმნიზედური შეკუმშვა)
    const temporalCompactingFixes = [
        [/(?<![\u10A0-\u10FF])საათების\s+განმავლობაში(?![ა-ჰ])/g, 'საათობით'],
        [/(?<![\u10A0-\u10FF])დიდი\s+დროის\s+განმავლობაში(?![ა-ჰ])/g, 'დიდხანს'],
        [/(?<![\u10A0-\u10FF])წლების\s+განმავლობაში(?![ა-ჰ])/g, 'წლობით'],
        [/(?<![\u10A0-\u10FF])ერთი\s+მომენტისთვის(?![ა-ჰ])/g, 'წამით'],
        [/(?<![\u10A0-\u10FF])დღიდან\s+დღემდე(?![ა-ჰ])/g, 'დღითი დღე'],
    ];
    for (const [re, repl] of temporalCompactingFixes) {
        out = out.replace(re, repl);
    }

    // 51. Psychological & Mental State Collocations (ფსიქოლოგიური და მენტალური მდგომარეობების იდიომები)
    const mentalIdiomFixes = [
        [/(?<![\u10A0-\u10FF])(?:(?:მისი|თავისი)\s+)?გონება\s+დაკარგა(?![ა-ჰ])/g, 'ჭკუიდან შეიშალა'],
        [/(?<![\u10A0-\u10FF])დაკარგა\s+(?:(?:მისი|თავისი)\s+)?გონება(?![ა-ჰ])/g, 'ჭკუიდან შეიშალა'],
        [/(?<![\u10A0-\u10FF])(?:გააკეთა|შეადგინა)\s+(?:(?:მისი|თავისი)\s+)?გონება(?!\s+ერთხელ\s+და\s+ყველასთვის)(?![ა-ჰ])/g, 'გადაწყვიტა'],
        [/(?<![\u10A0-\u10FF])შეცვალა\s+(?:(?:მისი|თავისი)\s+)?გონება(?![ა-ჰ])/g, 'გადაიფიქრა'],
    ...
    for (const [re, repl] of epistemicModalFixes) {
        out = out.replace(re, repl);
    }

    // 59. Temporal Duratives & Inceptives (დროითი დურატივები და განგრძობადობა)
    const temporalDurativeFixes = [
        [/(?<![\u10A0-\u10FF])(?:მთელი\s+დღე\s+გრძელი|მთელი\s+დღის\s+გასწვრივ)(?![ა-ჰ])/g, 'მთელი დღის განმავლობაში'],
        [/(?<![\u10A0-\u10FF])დროის\s+კურსში(?![ა-ჰ])/g, 'დროთა განმავლობაში'],
        [/(?<![\u10A0-\u10FF])დროიდან\s+დროში(?![ა-ჰ])/g, 'დროდადრო'],
    ];
    for (const [re, repl] of temporalDurativeFixes) {
        out = out.replace(re, repl);
    }

    // 60. Adversative & Concessive Antithesis (შეპირისპირებითი და დათმობითი ანტითეზა)
    const adversativeAntithesisFixes = [
        [/(?<![\u10A0-\u10FF])საპირისპიროზე(?![ა-ჰ])/g, 'პირიქით'],
        [/(?<![\u10A0-\u10FF])ერთ\s+ხელზე(?![ა-ჰ])/g, 'ერთი მხრივ'],
        [/(?<![\u10A0-\u10FF])მეორე\s+ხელზე(?![ა-ჰ])/g, 'მეორე მხრივ'],
        [/(?<![\u10A0-\u10FF])თანაბრად\s+ასე(?![ა-ჰ])/g, 'მიუხედავად ამისა'],
    ...
    for (const [re, repl] of adversativeAntithesisFixes) {
        out = out.replace(re, repl);
    }

    // 61. Auditory Verbs & Sound Symbolism (აკუსტიკური ზმნები და ხმოვანი რეფლექსები)
    const auditorySoundFixes = [
        [/(?<![\u10A0-\u10FF])კარი\s+(?:დაეჯახა\s+დახურულად|დაეჯახა)(?![ა-ჰ])/g, 'კარი გაჯახუნდა'],
        [/(?<![\u10A0-\u10FF])იატაკი\s+(?:ყვიროდა|გამოსცემდა\s+ხმას)(?![ა-ჰ])/g, 'იატაკი აჭრაჭუნდა'],
        [/(?<![\u10A0-\u10FF])(?:მისი\s+)?კბილები\s+(?:ლაპარაკობდნენ|აკაკუნებდნენ)(?![ა-ჰ])/g, 'კბილი კბილზე ადიოდა'],
        [/(?<![\u10A0-\u10FF])ნაბიჯები\s+ექოდ\s+ისმოდა(?![ა-ჰ])/g, 'ნაბიჯების ხმა ექოდ გაისმა'],
    ...
    for (const [re, repl] of auditorySoundFixes) {
        out = out.replace(re, repl);
    }

    // 62. Emotional & Cognitive Metaphors (ემოციური და აფექტური მეტაფორები)
    const emotionalMetaphorFixes = [
        [/(?<![\u10A0-\u10FF])(?:დაკარგა\s+ტემპერამენტი|ტემპერამენტი\s+დაკარგა)(?![ა-ჰ])/g, 'მოთმინება დაკარგა'],
        [/(?<![\u10A0-\u10FF])შეინარჩუნა\s+სიგრილე(?![ა-ჰ])/g, 'სიმშვიდე შეინარჩუნა'],
        [/(?<![\u10A0-\u10FF])დაჯდა\s+მის\s+ნერვებზე(?![ა-ჰ])/g, 'ნერვებს უშლიდა'],
        [/(?<![\u10A0-\u10FF])გული\s+დადო\s+ამაზე(?![ა-ჰ])/g, 'გულით მოინდომა'],
    ...
    for (const [re, repl] of emotionalMetaphorFixes) {
        out = out.replace(re, repl);
    }

    // 63. Physical Posture & Stative Orientation (სხეულის პოზა და მდგომარეობის ზმნები)
    const postureStativeFixes = [
        [/(?<![\u10A0-\u10FF])იჯდა\s+გადაჯვარედინებული\s+ფეხებით(?![ა-ჰ])/g, 'ფეხმორთხმით იჯდა'],
        [/(?<![\u10A0-\u10FF])იდგა\s+ფეხის\s+თითებზე(?![ა-ჰ])/g, 'ფეხის წვერებზე იდგა'],
        [/(?<![\u10A0-\u10FF])დაეცა\s+(?:მის\s+)?მუხლებზე(?![ა-ჰ])/g, 'მუხლებზე დაემხო'],
        [/(?<![\u10A0-\u10FF])გადააჯვარედინა\s+(?:თავისი\s+)?ხელები(?![ა-ჰ])/g, 'გულზე ხელები დაიკრიფა'],
    ...
    for (const [re, repl] of postureStativeFixes) {
        out = out.replace(re, repl);
    }

    // 64. Quantificational Intensifiers & Degree Particles (ინტენსიფიკატორები და ზმნიზედური ხარისხები)
    const quantifierDegreeFixes = [
        [/(?<![\u10A0-\u10FF])დიდით\s+და\s+ვრცელით(?![ა-ჰ])/g, 'მთლიანობაში'],
        [/(?<![\u10A0-\u10FF])ერთხელ\s+ლურჯ\s+მთვარეზე(?![ა-ჰ])/g, 'ძალზე იშვიათად'],
        [/(?<![\u10A0-\u10FF])გასწვრივ\s+და\s+გასწვრივ(?![ა-ჰ])/g, 'თავიდან ბოლომდე'],
        [/(?<![\u10A0-\u10FF])უსაფრთხოდ\s+და\s+ხმით(?![ა-ჰ])/g, 'ცოცხალ-ჯანსაღი'],
    ...
    for (const [re, repl] of quantifierDegreeFixes) {
        out = out.replace(re, repl);
    }

    // 65. Polysemous Motion Collocations & Phrasal Directionals (მიმართულებითი და მოძრაობითი იდიომები)
    const motionDirectionalFixes = [
        [/(?<![\u10A0-\u10FF])მობრუნდა\s+უკან(?![ა-ჰ])/g, 'გამობრუნდა'],
        [/(?<![\u10A0-\u10FF])მოშორებით\s+შებრუნდა(?![ა-ჰ])/g, 'ზურგი შეაქცია'],
        [/(?<![\u10A0-\u10FF])მოშორებით\s+შეხედა(?![ა-ჰ])/g, 'მზერა აარიდა'],
        [/(?<![\u10A0-\u10FF])მზერა\s+ესროლა(?![ა-ჰ])/g, 'თვალი შეავლო'],
    ...
    for (const [re, repl] of motionDirectionalFixes) {
        out = out.replace(re, repl);
    }

    // 66. Environmental & Meteorological Verbs (ატმოსფერულ-მეტეოროლოგიური ზმნები და უპირო ბუნებრივი მოვლენები)
    const meteorologicalFixes = [
        [/(?<![\u10A0-\u10FF])(?:წვიმამ\s+დაიწყო\s+მძიმედ\s+წვიმა|წვიმა\s+წამოვიდა\s+მძიმედ)(?![ა-ჰ])/g, 'კოკისპირულად გაწვიმდა'],
        [/(?<![\u10A0-\u10FF])ჭექა-ქუხილმა\s+გააგორა\s+მთებზე(?![ა-ჰ])/g, 'მთებში ქუხილმა დაიგრგვინა'],
        [/(?<![\u10A0-\u10FF])ნისლი\s+დაჯდა\s+ხეობაზე(?![ა-ჰ])/g, 'ხეობას ნისლი ჩამოწვა'],
        [/(?<![\u10A0-\u10FF])ქარი\s+ყვიროდა\s+ხეებში(?![ა-ჰ])/g, 'ქარი ხეებში ღმუოდა'],
    ...
    for (const [re, repl] of meteorologicalFixes) {
        out = out.replace(re, repl);
    }

    // 67. Phrasal Resultatives & State-Change Aspectuals (შედეგობრივი ზმნები და მდგომარეობის შეცვლის ასპექტი)
    const resultativeAspectFixes = [
        [/(?<![\u10A0-\u10FF])(?:ცეცხლმა\s+თავი\s+დაწვა|ცეცხლი\s+დაიწვა\s+თვითონ)(?![ა-ჰ])/g, 'ცეცხლი ჩაქრა'],
        [/(?<![\u10A0-\u10FF])მდინარე\s+გაიქცა\s+მშრალი(?![ა-ჰ])/g, 'მდინარე დაშრა'],
        [/(?<![\u10A0-\u10FF])საათმა\s+ტიკტიკით\s+გაუშვა\s+წუთები(?![ა-ჰ])/g, 'საათი წუთებს ითვლიდა'],
        [/(?<![\u10A0-\u10FF])შუშა\s+დაიმსხვრა\s+ნაჭრებში(?![ა-ჰ])/g, 'შუშა ნამსხვრევებად იქცა'],
    ...
    for (const [re, repl] of resultativeAspectFixes) {
        out = out.replace(re, repl);
    }

    // 68. Psychological Volition & Dispositional Adjectives (ნებელობით-ფსიქოლოგიური განწყობისა და მიდრეკილების სინთეზი)
    const psychologicalDispositionFixes = [
        [/(?<![\u10A0-\u10FF])(?:(?:იყო\s+)?სიტყვების\s+(?:დანაკარგში|დაკარგვაში)(?:\s+იყო)?)(?![ა-ჰ])/g, 'სიტყვა ვეღარ მოეძებნა'],
        [/(?<![\u10A0-\u10FF])(?:(?:შეადგინა\s+თავისი\s+გონება|გადაწყვიტა)\s+ერთხელ\s+და\s+ყველასთვის|გონება\s+ერთხელ\s+და\s+ყველასთვის)(?![ა-ჰ])/g, 'საბოლოოდ გადაწყვიტა'],
        [/(?<![\u10A0-\u10FF])ჰქონდა\s+ცუდი\s+გრძნობა\s+ამაზე(?![ა-ჰ])/g, 'ცუდი წინათგრძნობა ჰქონდა'],
        [/(?<![\u10A0-\u10FF])(?:ეს\s+გულთან\s+აიღო|აიღო\s+ეს\s+გულში)(?![ა-ჰ])/g, 'გულთან ახლოს მიიტანა'],
    ...
    for (const [re, repl] of psychologicalDispositionFixes) {
        out = out.replace(re, repl);
    }

    // 69. Cognitive Perception & Realization Idioms (შემეცნებითი გააზრებისა და აღქმის იდიომები)
    const cognitiveRealizationFixes = [
        [/(?<![\u10A0-\u10FF])ეს\s+გათენდა\s+მასზე,\s*რომ(?![ა-ჰ])/g, 'უეცრად მიხვდა, რომ'],
        [/(?<![\u10A0-\u10FF])მოვიდა\s+თავის\s+გრძნობებში(?![ა-ჰ])/g, 'გონს მოეგო'],
        [/(?<![\u10A0-\u10FF])დაინახა\s+მოტყუების\s+გავლით(?![ა-ჰ])/g, 'სიცრუე მაშინვე ამოიცნო'],
        [/(?<![\u10A0-\u10FF])მიიღო\s+გარანტირებულად(?![ა-ჰ])/g, 'თავისთავად ცხადად მიიჩნია'],
    ...
    for (const [re, repl] of cognitiveRealizationFixes) {
        out = out.replace(re, repl);
    }

    // 70. Collocational Intensifiers & Bound Spatial Preverbs (გამაძლიერებელი კოლოკაციები და შეკრული სივრცითი ზმნისწინები)
    const spatialIntensifierFixes = [
        [/(?<![\u10A0-\u10FF])ღრმად\s+ფიქრში(?![ა-ჰ])/g, 'ფიქრებში ჩაძირული'],
        [/(?<![\u10A0-\u10FF])ლურჯიდან\s+გარეთ(?![ა-ჰ])/g, 'მოულოდნელად'],
        [/(?<![\u10A0-\u10FF])ყველა\s+შანსების\s+წინააღმდეგ(?![ა-ჰ])/g, 'ყოველგვარი დაბრკოლების მიუხედავად'],
        [/(?<![\u10A0-\u10FF])ეჭვის\s+ჩრდილის\s+მიღმა(?![ა-ჰ])/g, 'ეჭვგარეშეა'],
    ...
    for (const [re, repl] of spatialIntensifierFixes) {
        out = out.replace(re, repl);
    }

    // 71. Sensory Perception & Somatic Collocations (გრძნობითი აღქმისა და სომატური შეგრძნებების იდიომები)
    const sensorySomaticFixes = [
        [/(?<![\u10A0-\u10FF])(?:მძაფრი\s+სუნი\s+ეკიდა|სუნი\s+ეკიდა\s+ჰაერში)(?![ა-ჰ])/g, 'მძაფრი სუნი იდგა'],
        [/(?<![\u10A0-\u10FF])სიცივემ\s+გაიარა\s+მის\s+ხერხემალში(?![ა-ჰ])/g, 'ტანში ცივმა ჟრუანტელმა დაუარა'],
        [/(?<![\u10A0-\u10FF])ზარი\s+მის\s+ყურებში(?![ა-ჰ])/g, 'ყურებში წუილი'],
        [/(?<![\u10A0-\u10FF])ყურებში\s+რეკვა\s+დაიწყო(?![ა-ჰ])/g, 'ყურები აუწუილდა'],
    ...
    for (const [re, repl] of sensorySomaticFixes) {
        out = out.replace(re, repl);
    }

    // 72. Epistemic Evidentiality & Discourse Pragmatic Connectives (ეპისტემური მოდალობა და დისკურსული კონექტორები)
    const epistemicDiscourseFixes = [
        [/(?<![\u10A0-\u10FF])როგორც\s+ფაქტის\s+საკითხი(?![ა-ჰ])/g, 'სინამდვილეში'],
        [/(?<![\u10A0-\u10FF])ყველა\s+ანგარიშით(?![ა-ჰ])/g, 'როგორც ჩანს'],
        [/(?<![\u10A0-\u10FF])არ\s+აქვს\s+მნიშვნელობა\s+რა\s+მოხდება(?![ა-ჰ])/g, 'რაც არ უნდა მოხდეს'],
        [/(?<![\u10A0-\u10FF])სხვა\s+სიტყვებში(?![ა-ჰ])/g, 'სხვა სიტყვებით რომ ვთქვათ'],
    ...
    for (const [re, repl] of epistemicDiscourseFixes) {
        out = out.replace(re, repl);
    }

    // 73. Dynamic Motion & Spatial Transit Idioms (დინამიკური მოძრაობისა და ტრანზიტის იდიომები)
    const motionTransitFixes = [
        [/(?<![\u10A0-\u10FF])(?:გაიქცა\s+უკან\s+ყურების\s+გარეშე|უკან\s+ყურების\s+გარეშე\s+გაიქცა)(?![ა-ჰ])/g, 'უკანმოუხედავად გაიქცა'],
        [/(?<![\u10A0-\u10FF])(?:გააკეთა\s+თავისი\s+გზა|თავისი\s+გზა\s+გააკეთა)(?![ა-ჰ])/g, 'გზა გაიკვლია'],
        [/(?<![\u10A0-\u10FF])მიჰყვა\s+ცხელ\s+ქუსლებზე(?![ა-ჰ])/g, 'ფეხდაფეხ მიჰყვებოდა'],
        [/(?<![\u10A0-\u10FF])გაიარა\s+გვერდით\s+გარშემო(?![ა-ჰ])/g, 'გვერდი აუარა'],
    ...
    for (const [re, repl] of motionTransitFixes) {
        out = out.replace(re, repl);
    }

    // 74. Conversational Interlocution & Dialogue Discourse (დიალოგური რეპლიკაციისა და მეტყველების აქტის ზმნები)
    const dialogueLocutionFixes = [
        [/(?<![\u10A0-\u10FF])(?:გაჭრა\s+მისი\s+სიტყვა|შეაწყვეტინა\s+იგი)(?![ა-ჰ])/g, 'სიტყვა შეაწყვეტინა'],
        [/(?<![\u10A0-\u10FF])დაარტყა\s+საუბარი(?![ა-ჰ])/g, 'საუბარი გააბა'],
        [/(?<![\u10A0-\u10FF])ილაპარაკა\s+უკან(?![ა-ჰ])/g, 'სიტყვა შეუბრუნა'],
        [/(?<![\u10A0-\u10FF])გამოხატა\s+აზრი(?![ა-ჰ])/g, 'აზრი გამოთქვა'],
    ...
    for (const [re, repl] of dialogueLocutionFixes) {
        out = out.replace(re, repl);
    }

    // 75. Existential State Transitions & Temporal Duratives (დროითი ხანგრძლივობისა და ყოფიერების ასპექტური სტრუქტურები)
    const existentialDurativeFixes = [
        [/(?<![\u10A0-\u10FF])დრო\s+გადიოდა\s+გარეთ(?![ა-ჰ])/g, 'დრო მიდიოდა'],
        [/(?<![\u10A0-\u10FF])დღის\s+გავლით(?![ა-ჰ])/g, 'მთელი დღის განმავლობაში'],
        [/(?<![\u10A0-\u10FF])(?:გაიყინა\s+ადგილზე\s+ერთ\s+წამში|გაიყინა\s+ადგილზე)(?![ა-ჰ])/g, 'წამიერად გაქვავდა'],
        [/(?<![\u10A0-\u10FF])დაიჭირა\s+მომენტი(?![ა-ჰ])/g, 'დრო იხელთა'],
    ...
    for (const [re, repl] of existentialDurativeFixes) {
        out = out.replace(re, repl);
    }

    // 76. Environmental Light, Shadow & Atmospheric Illumination (სინათლის, ჩრდილისა და ციური განათების კოლოკაციები)
    const lightShadowFixes = [
        [/(?<![\u10A0-\u10FF])(?:მზე\s+ჩადიოდა\s+ჰორიზონტზე|მზე\s+ჰორიზონტზე\s+ჩადიოდა)(?![ა-ჰ])/g, 'მზე ჰორიზონტს ეფარებოდა'],
        [/(?<![\u10A0-\u10FF])(?:ჩრდილი\s+დაეცა\s+მიწაზე|ჩრდილი\s+დაეცა\s+მიწას)(?![ა-ჰ])/g, 'მიწას ჩრდილი დაადგა'],
        [/(?<![\u10A0-\u10FF])მთვარე\s+ანათებდა\s+კაშკაშად(?![ა-ჰ])/g, 'მთვარე მკვეთრად ანათებდა'],
        [/(?<![\u10A0-\u10FF])(?:ბინდი\s+დაეცა\s+ქალაქს|ბინდი\s+დააწვა\s+ქალაქს)(?![ა-ჰ])/g, 'ქალაქს ბინდი ჩამოაწვა'],
        [/(?<![\u10A0-\u10FF])(?:სხივებმა\s+გაჭრა\s+ღრუბლები|სხივებმა\s+გაარღვია\s+ღრუბლები)(?![ა-ჰ])/g, 'სხივებმა ღრუბლებში გამოაღწია'],
        [/(?<![\u10A0-\u10FF])დილის\s+სინათლე\s+გატყდა\s+ფანჯარაში(?![ა-ჰ])/g, 'დილის სინათლემ ფანჯარაში შემოაღწია'],
    ];
    for (const [re, repl] of lightShadowFixes) {
        out = out.replace(re, repl);
    }

    // 77. Causative-Inchoative Ergative Alternations (კაუზატიურ-ინქოატიური ერგატიული მონაცვლეობა)
    const causativeInchoativeFixes = [
        [/(?<![\u10A0-\u10FF])კარი\s+თავისით\s+გაიღო(?![ა-ჰ])/g, 'კარი გაიღო'],
        [/(?<![\u10A0-\u10FF])ფანჯარა\s+გატყდა\s+თვითონ(?![ა-ჰ])/g, 'ფანჯარა გატყდა'],
        [/(?<![\u10A0-\u10FF])ყინული\s+გადნა\s+თავისით(?![ა-ჰ])/g, 'ყინული გადნა'],
        [/(?<![\u10A0-\u10FF])სახლი\s+დაიწვა\s+თვითონ(?![ა-ჰ])/g, 'სახლი დაიწვა'],
        [/(?<![\u10A0-\u10FF])წყალმა\s+დაიხრჩო\s+იგი(?![ა-ჰ])/g, 'წყალში დაიხრჩო'],
        [/(?<![\u10A0-\u10FF])ტოტი\s+გატყდა\s+თავისით(?![ა-ჰ])/g, 'ტოტი გადატყდა'],
    ];
    for (const [re, repl] of causativeInchoativeFixes) {
        out = out.replace(re, repl);
    }

    // 78. Intransitive Directional Particle Preverbs (მიმართულებითი ზმნისწინის სიჭარბის აღმოფხვრა)
    const directionalParticleFixes = [
        [/(?<![\u10A0-\u10FF])(?:მოვიდა\s+შიგნით\s+ოთახში|მოვიდა\s+ოთახში)(?![ა-ჰ])/g, 'ოთახში შემოვიდა'],
        [/(?<![\u10A0-\u10FF])მოვიდა\s+შიგნით(?![ა-ჰ])/g, 'შემოვიდა'],
        [/(?<![\u10A0-\u10FF])წავიდა\s+გარეთ\s+ბაღში(?![ა-ჰ])/g, 'ბაღში გავიდა'],
        [/(?<![\u10A0-\u10FF])წავიდა\s+გარეთ(?![ა-ჰ])/g, 'გავიდა'],
        [/(?<![\u10A0-\u10FF])ჩამოვიდა\s+დაბლა\s+კიბეზე(?![ა-ჰ])/g, 'კიბეზე ჩამოვიდა'],
        [/(?<![\u10A0-\u10FF])ჩამოვიდა\s+დაბლა(?![ა-ჰ])/g, 'დაეშვა'],
        [/(?<![\u10A0-\u10FF])ავიდა\s+ზემოთ\s+კოშკში(?![ა-ჰ])/g, 'კოშკში ავიდა'],
        [/(?<![\u10A0-\u10FF])ავიდა\s+ზემოთ(?![ა-ჰ])/g, 'ავიდა'],
        [/(?<![\u10A0-\u10FF])გადავიდა\s+მეორე\s+მხარეს(?![ა-ჰ])/g, 'გადავიდა'],
        [/(?<![\u10A0-\u10FF])გამოვიდა\s+გარეთ\s+ეზოში(?![ა-ჰ])/g, 'ეზოში გამოვიდა'],
        [/(?<![\u10A0-\u10FF])გამოვიდა\s+გარეთ(?![ა-ჰ])/g, 'გამოვიდა'],
    ];
    for (const [re, repl] of directionalParticleFixes) {
        out = out.replace(re, repl);
    }

    // 79. Phrasal Intensification & Evaluative Adverbial Clitics (ზმნიზედური ინტენსიფიკატორები და შეფასებითი კლიტიკები)
    const evaluativeIntensifierFixes = [
        [/(?<![\u10A0-\u10FF])სრულიად\s+აშკარად(?![ა-ჰ])/g, 'ცხადლივ'],
        [/(?<![\u10A0-\u10FF])ძალიან\s+მარტივად(?![ა-ჰ])/g, 'ძალზე იოლად'],
        [/(?<![\u10A0-\u10FF])უკიდურესად\s+რთული(?![ა-ჰ])/g, 'უაღრესად რთული'],
        [/(?<![\u10A0-\u10FF])საერთოდ\s+არ\s+ეშინოდა(?![ა-ჰ])/g, 'სულაც არ ეშინოდა'],
        [/(?<![\u10A0-\u10FF])საერთოდ\s+არ(?![ა-ჰ])/g, 'სულაც არ'],
        [/(?<![\u10A0-\u10FF])თითქმის\s+შეუძლებელი\s+იყო(?![ა-ჰ])/g, 'ფაქტობრივად შეუძლებელი იყო'],
        [/(?<![\u10A0-\u10FF])თითქმის\s+შეუძლებელი(?![ა-ჰ])/g, 'ფაქტობრივად შეუძლებელი'],
        [/(?<![\u10A0-\u10FF])აბსოლუტურად\s+დარწმუნებული(?![ა-ჰ])/g, 'სავსებით დარწმუნებული'],
    ];
    for (const [re, repl] of evaluativeIntensifierFixes) {
        out = out.replace(re, repl);
    }

    // 80. Classical Literary Narrative Sentence Starters & Connectives (კლასიკური ნარატიული ინციპიტები და დისკურსული გადასვლები)
    const narrativeConnectiveFixes = [
        [/(?<![\u10A0-\u10FF])და\s+მერე\s+მან\s+თქვა(?![ა-ჰ])/g, 'შემდეგ კი თქვა'],
        [/(?<![\u10A0-\u10FF])და\s+მერე\s+თქვა(?![ა-ჰ])/g, 'შემდეგ კი თქვა'],
        [/(?<![\u10A0-\u10FF])როდესაც\s+ყველაფერი\s+დასრულდა(?![ა-ჰ])/g, 'როცა ყველაფერი მიწყნარდა'],
        [/(?<![\u10A0-\u10FF])არც\s+კი\s+შეუხედავს(?![ა-ჰ])/g, 'თვალიც არ შეუკრავს'],
        [/(?<![\u10A0-\u10FF])ამასობაში\s+კი(?![ა-ჰ])/g, 'ამასობაში'],
        [/(?<![\u10A0-\u10FF])ერთი\s+სიტყვით\s+რომ\s+ვთქვათ(?![ა-ჰ])/g, 'მოკლედ რომ ვთქვათ'],
    ];
    for (const [re, repl] of narrativeConnectiveFixes) {
        out = out.replace(re, repl);
    }
...
// ── 4.7. DETERMINISTIC IN-HOUSE OFFLINE TRANSLATION ENGINE (ZERO LLM) ────────
...
function translateOfflineEnToKa(text) {
    ...
    const translatedParas = paragraphs.map(para => {
        ...
        const phraseMap = [
            ...
            // Prepositional phrases with synthetic case assignment
            [/\bto\s+the\s+little\s+prince\b/gi, 'პატარა უფლისწულს'],
            [/\bto\s+the\s+prince\b/gi, 'უფლისწულს'],
            [/\bto\s+the\s+king\b/gi, 'მეფეს'],
            [/\bto\s+the\s+fox\b/gi, 'მელას'],
            [/\bto\s+the\s+rose\b|\bto\s+the\s+flower\b/gi, 'ვარდს'],
            [/\bto\s+the\s+child\b/gi, 'ბავშვს'],
            [/\bto\s+the\s+man\b/gi, 'კაცს'],
            [/\bto\s+the\s+woman\b/gi, 'ქალს'],
            [/\bto\s+me\b/gi, 'მე'],
            [/\bto\s+you\b/gi, 'შენ'],
            [/\bto\s+him\b|\bto\s+her\b/gi, 'მას'],
            [/\bto\s+us\b/gi, 'ჩვენ'],
            [/\bto\s+them\b/gi, 'მათ'],

            // Possessives with Genitive case
            [/\bof\s+the\s+little\s+prince\b/gi, 'პატარა უფლისწულის'],
            [/\bof\s+the\s+prince\b/gi, 'უფლისწულის'],
            [/\bof\s+the\s+king\b/gi, 'მეფის'],
            [/\bof\s+the\s+fox\b/gi, 'მელის'],
            [/\bof\s+the\s+rose\b|\bof\s+the\s+flower\b/gi, 'ვარდის'],
            [/\bof\s+the\s+world\b/gi, 'სამყაროს'],
            [/\bof\s+the\s+heart\b/gi, 'გულის'],
            [/\bof\s+the\s+stars\b/gi, 'ვარსკვლავების'],
            [/\bof\s+the\s+mountain\b/gi, 'მთის'],
            [/\bof\s+the\s+desert\b/gi, 'უდაბნოს'],
            [/\bof\s+the\s+sun\b/gi, 'მზის'],
            [/\bof\s+the\s+moon\b/gi, 'მთვარის'],
            [/\bof\s+the\s+sky\b/gi, 'ცის'],
            [/\bof\s+the\s+sea\b/gi, 'ზღვის'],
            [/\bof\s+the\s+earth\b/gi, 'დედამიწის'],
            [/\bof\s+life\b/gi, 'ცხოვრების'],
            [/\bof\s+death\b/gi, 'სიკვდილის'],

            // Locative phrases with postpositions
            [/\bin\s+the\s+desert\b/gi, 'უდაბნოში'],
            [/\bin\s+the\s+sky\b/gi, 'ცაში'],
            [/\bin\s+the\s+world\b/gi, 'სამყაროში'],
            [/\bin\s+the\s+room\b/gi, 'ოთახში'],
            [/\bin\s+the\s+house\b/gi, 'სახლში'],
            [/\bin\s+the\s+forest\b/gi, 'ტყეში'],
            [/\bin\s+the\s+city\b/gi, 'ქალაქში'],
            [/\bin\s+the\s+garden\b/gi, 'ბაღში'],
            [/\bin\s+the\s+heart\b/gi, 'გულში'],
            [/\bon\s+the\s+planet\b/gi, 'პლანეტაზე'],
            [/\bon\s+the\s+earth\b/gi, 'დედამიწაზე'],
            [/\bon\s+the\s+mountain\b/gi, 'მთაზე'],
            [/\bon\s+the\s+table\b/gi, 'მაგიდაზე'],
            [/\bon\s+the\s+way\b|\bon\s+the\s+road\b/gi, 'გზაზე'],
            [/\bfrom\s+the\s+heart\b/gi, 'გულიდან'],
            [/\bfrom\s+the\s+stars\b/gi, 'ვარსკვლავებიდან'],
            [/\bfrom\s+the\s+sky\b/gi, 'ციდან'],
            [/\bfrom\s+the\s+earth\b/gi, 'მიწიდან'],

            // Narrative verbs
            [/\bsmiled\b/gi, 'გაიღიმა'],
            [/\blaughed\b/gi, 'გაიცინა'],
            [/\bwept\b|\bcried\b/gi, 'ატირდა'],
            [/\bsighed\b/gi, 'ამოიოხრა'],
            [/\bshouted\b/gi, 'დაიყვირა'],
            [/\bwhispered\b/gi, 'ჩაიჩურჩულა'],
            [/\bmurmured\b/gi, 'ჩაიბუტბუტა'],
            [/\bnodded\b/gi, 'თავი დაუქნია'],
            [/\bhesitated\b/gi, 'შეყოყმანდა'],
            [/\bdecided\b/gi, 'გადაწყვიტა'],
            [/\bwondered\b/gi, 'გაუკვირდა'],
            [/\bdiscovered\b/gi, 'აღმოაჩინა'],
            [/\bcreated\b/gi, 'შექმნა'],
            [/\bprotected\b/gi, 'დაიცვა'],
            [/\bremembered\b/gi, 'გაახსენდა'],
            [/\bforgot\b/gi, 'დაავიწყდა'],
            [/\bdisappeared\b/gi, 'გაუჩინარდა'],
            [/\bappeared\b/gi, 'გამოჩნდა'],
            [/\bwandered\b/gi, 'დახეტიალობდა'],
        ...
        // 4. Run through standard morphology auto-corrections & morphosyntactic synthesis
        p = correctGeorgianMorphology(p);
        p = synthesizeGeorgianMorphology(p);
...
const GEORGIAN_KNOWLEDGE_VERSION = '1.96.0';
