...
(function (root, factory) {
    ...
    function assessTranslation(source, output, language) {
        ...
        if (target === 'ka') {
            ...
            if (/(?<![\u10A0-\u10FF])(?:გამოიყურებოდა\s+გარეგნულად|თავის\s+გარეგნულ\s+ბიძგში|ქვებივით\s+აფრინდა\s+მათ|გიმ(?:რ|კრ)ეკის|საკმარისად\s+იყო\s+ნაპოვნი|თავსატეხების\s+ყუთებ|უაზრობის\s+კოშმარი\s+უსასრულოდ|უგემოვნო\s+ზღვაში|უცოდინარი\s+ჭეშმარიტებების|უბიძგებდა\s+მუდამ\s+გარეგნულად|თავსატეხების\s+ყუთები\s+მათში|ადამიანებს\s+არ\s+ჰქონდათ\s+მარტივი\s+წვდომა)(?![\u10A0-\u10FF])/u.test(out)) {
