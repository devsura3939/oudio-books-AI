(function(root, factory) {
    ...
    function promptFor(stage, {source, draft, baseline, target, before, after, glossary, issues = []}) {
        ...
        const rules = target === 'ka' ? 'Use natural Georgian syntax with topic-focus preverbal word order and authentic literary screeves, not English word-order calques. Transitive and medial Series II Aorist verbs require Ergative subjects (-მა); Series III evidentials require Dative subjects (-ს: ავტორს დაუწერია, მეფეს უბრძანებია); experiencer verbs take Dative subjects (მას უნდა/უყვარს/ახსოვს). Nouns modified by numerals or quantifiers (ორი, სამი, ათი, მრავალი, რამდენიმე) must remain singular (სამი წიგნი, ათი დღე). Prefer synthetic privative adverbs with უ-...-ოდ (უეჭველად, უშიშრად, უიმედოდ, უხმოდ). Avoid overusing გაკეთება (use შეცდომის დაშვება, გავლენის მოხდენა). Use version markers (დაიწერა წერილი, შვილს აუშენა) and ablative -გან (ღვთისგან ბოძებული). Use participial clauses (დაწერილი წიგნი, მომავალი თაობა) instead of clumsy რომელიც clauses. Negative imperatives must use prohibitive particle ნუ (ნუ წახვალ, ნუ გეშინია, ნუ ტირი), never declarative *არ წახვიდე. Adjectives in oblique cases drop nominative -ი before nouns (უცნობ სივრცეში, დიდ სამყაროში). Respect deictic preverbs (მოვიდა vs წავიდა) and bound dialogue quotatives (-ო, მეთქი, თქო). Resolve pronouns from context and preserve literary register.' : 'Use idiomatic English with source-faithful tense, pronouns and punctuation.';
