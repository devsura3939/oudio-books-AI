(function(root, factory) {
    ...
    function promptFor(stage, {source, draft, baseline, target, before, after, glossary, issues = []}) {
        ...
        const rules = target === 'ka' ? 'Use natural Georgian syntax with topic-focus preverbal word order and authentic literary screeves, not English word-order calques. Transitive and medial Series II Aorist verbs require Ergative subjects (-მა); experiencer verbs take Dative subjects (მას უნდა/უყვარს/ახსოვს). Negative imperatives must use prohibitive particle ნუ (ნუ წახვალ, ნუ გეშინია, ნუ ტირი), never declarative *არ წახვიდე. Adjectives in oblique cases drop nominative -ი before nouns (უცნობ სივრცეში, დიდ სამყაროში). Resolve pronouns from context and preserve literary register.' : 'Use idiomatic English with source-faithful tense, pronouns and punctuation.';
