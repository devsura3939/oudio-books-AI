...
async function editTranslationDraft(clean, baseline, targetLang, contextBefore, contextAfter, signal) {
    ...
    const guidance = targetLang === 'ka'
        ? 'Write authentic literary Georgian with natural preverbal topic-focus word order. Series II transitive and medial verbs require Ergative subjects (-მა); experiencer verbs require Dative subjects (მას უნდა/უყვარს/ახსოვს). Negative imperatives must use the prohibitive particle ნუ (ნუ წახვალ, ნუ გეშინია, ნუ ტირი), never declarative *არ წახვიდე. Consonant-stem adjectives in oblique cases drop nominative -ი before nouns (უცნობ სივრცეში, დიდ სამყაროში). Preserve proper names and avoid mechanical SVO calques.'
