...
with open(os.path.join(REPO_DIR, "index.html"), "r", encoding="utf-8") as f:
    root_html = f.read()
assert "v=1.48.2" in root_html, "FAIL: index.html missing current engine cache buster"
assert "v1.48.2" in root_html, "FAIL: index.html missing version text"
...
with open(os.path.join(REPO_DIR, "lovable-app", "public", "studio", "index.html"), "r", encoding="utf-8") as f:
    studio_html = f.read()
assert "v=1.48.2" in studio_html, "FAIL: studio/index.html missing version cache buster"
assert "v1.48.2" in studio_html, "FAIL: studio/index.html missing version text"
...
for path_name, content in [("static/app.js", current_app_content), ("studio/static/app.js", current_studio_app_content)]:
    ...
    assert "v1.48.2" in content, f"FAIL: version missing from {path_name}"
    # provider fixes
...
for path_name, path in [("index.html", INDEX_HTML), ("studio/index.html", STUDIO_INDEX)]:
    ...
    assert "v1.48.2" in html_text, f"FAIL: Version v1.48.2 not reflected in {path_name}"
