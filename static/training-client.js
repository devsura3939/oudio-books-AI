(function (root, factory) {
    ...
    'use strict';
    async function createKey({ staticHost = false, language = 'ka', getAccessToken, fetchImpl = fetch, apiUrl = null } = {}) {
        if (!apiUrl && typeof window !== 'undefined') {
            apiUrl = window.LUMINA_RUNTIME_CONFIG?.API_URL || null;
        }
        if (staticHost && !apiUrl) throw new Error('Open Training Lab on the server-hosted app to create a working key. GitHub Pages cannot run the training API.');
        ...
        if (token) {
            response = await fetchImpl(`${base}/api/admin/training`, {
        ...
        if (!response || [404, 405].includes(response.status)) {
            response = await fetchImpl(`${base}/api/public/train/key/generate`, {
